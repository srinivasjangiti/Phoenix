...
**This file is the single source of truth for what every Phoenix button, widget, and
...
- **Semantics:** Carrier broadcasts `carrier_restarting` on WS to all clients,
  flushes PTY reconnect tokens to DB, then `process.exit(1)`. `phoenix-loop.bat`
...
- **Client behavior:** On `carrier_restarting` WS event, show a banner
  "Restarting Phoenix… reconnecting in ~3s", close WS, retry connect every 500ms
  until the new carrier answers, then reattach PTY via `reconnect_token`.
- **Label in UI:** "Restart Phoenix" (was previously "Reload Server (Craft Swap)"
...
- **Semantics:** Kills any process holding 7781, spawns
  `node service/dev-server.js` with `PHOENIX_DEV=1`, polls `/health` until healthy,
  returns.
...
## Terminal input bar → Call Phoenix button (phone-handset icon, between mic and model picker)

- **Purpose:** Open a live conversation with Phoenix in a floating Tauri window. The
  conversation surface reuses the existing Comms thread view, focused on the
  Phoenix system thread (`thread-phoenix-system`).
- **Calls:** `openPanCall()` in `terminal/+page.svelte` → POSTs to local Tauri
  server (`http://127.0.0.1:${TAURI_PORT}/open`) with URL
  `/v2/comms?view=contacts&thread=thread-phoenix-system&call=1`. Falls back to
  `window.open()` if Tauri isn't running.
- **Semantics:** Single conversational surface. Same thread renders:
  user-typed/voice-in messages, Phoenix reactive replies, Phoenix proactive interjections
  (via `phoenix-notify.js`), and Phoenix intuition traces (`body_type='phoenix_intuition_trace'`,
  collapsible like Claude tool-call rendering).
- **Preserves:** Phoenix thread continuity (single hardcoded thread ID across sessions,
...
- **Pre-gate:** None on click. Voice loop (when wired) will respect Phoenix's quiet
...
- **Pre-gate:** None for Phoenix thread. Phoenix↔Phoenix federated calls (future) go through
  `trust-ladder.js` evaluation.

---

## Comms thread → Intuition trace rendering (`body_type='phoenix_intuition_trace'`)

- **Purpose:** Show Phoenix's reasoning inline in the conversation, the same way
  Claude exposes tool-call traces.
- **Renders as:** `<details>` element with `🧠 Phoenix intuition · <time>` summary;
  click to expand the body (monospace, scrollable, dashed border).
- **Source:** Any service writing to `chat_messages` with
  `body_type='phoenix_intuition_trace'` (intuition engine, router, dream cycle).
...
- **Purpose:** Gate Phoenix↔Phoenix federated communication. Composes three axes
...
- **Schema:**
  ...
  - `chat_threads.scope_allowlist TEXT` — JSON `{ allowed: [...], denied: [...] }`
    per-thread allowlist. `null` = unrestricted (your own Phoenix, no gate).
...
**The concept:** Phoenix tests itself, finds what's failing, generates fixes, and
...
Each suite tests one Phoenix service against known inputs and expected outputs.
...
### 🔊 Intuition Test
Tests the voice router — Phoenix's primary interface. Uses the 7-score vocabulary
...
### 🧠 Memory Test
Tell Phoenix 10 facts via API → wait → query each fact → score recall.
...
**What Phoenix built is not just a feature. It is a pattern any AI application can adopt.**
...
Phoenix defined 7 axes for voice routing. Any application defines their own:

| Domain | Their "Hearing" | Their "Clarity" | Their "Reflex" |
|--------|----------------|----------------|---------------|
| Voice AI (Phoenix) | Garbled STT tolerance | JSON schema reliability | TTFT <400ms |
...
Phoenix notices at 3am and fixes it before anyone wakes up.

That's not just a feature — it's what makes an AI application trustworthy over time.
Other developers building on Phoenix's infrastructure get this loop for free. Define
...
### Philosophy
Phoenix identifies people through accumulated multi-modal evidence — never by
comparing against external face databases. All identity data lives in Phoenix's
own DB. The system builds confidence over time, not from a single image.

### Phoenix's Mind — first-person thought stream (dashboard Intuition section)

**What it is**: A live, persisted, first-person reasoning trace — modeled on
Claude's extended-thinking blocks. Every reasoning event in Phoenix writes ONE
...
**Storage**: `phoenix_thoughts(id, ts, source, thought, refs_json, importance)`.
Capped at 240 chars per row. Producers call `writeThought(source, text, refs, importance)`
from `service/src/thoughts.js`. The dashboard renders newest-first; Phoenix itself
reads the stream back via the `phoenix_thoughts` MCP tool to answer questions like
"what was I just doing 10 minutes ago?".

**Replaces**: The old "What Phoenix's mind is busy with" feed, which showed raw
...
**MCP tool**: `phoenix_thoughts {limit?, source?, since_ms?, write?}` — read or write.
...
**Storage**:
- `phoenix_needs(user_id, need_id, level, weight, decay_rate, last_satisfied_at, last_evaluated_at)`
  — one row per (user, need). UNIQUE constraint.
- `phoenix_need_events(ts, user_id, need_id, kind, delta, level_after, source, refs_json, note)`
...
3× lowers Nourishment weight; thanking Phoenix for it raises it. Urgency in the
scorer = `(100 - level) / 100 × weight`.

**Identity**: Currently resolved via face-id lock from webcam-watcher (falls
back to `'owner'` for single-user). Will be replaced by identity-fusion cluster
once `intuition/identity.js` lands.

**Endpoints**:
- `GET /api/v1/needs?user=owner` → all needs, sorted by urgency desc
- `GET /api/v1/needs/definitions` → static metadata (icons, labels, defaults)
- `GET /api/v1/needs/events?user=owner&need=nourishment&limit=20` → audit log
...
- **Recovery (in order):**
  ...
  3. If still stuck: logs incident, alerts via Phoenix Notify
...
- **Purpose:** When a client device installs Phoenix, name it from hardware model
  instead of raw hostname (e.g. "Dell G16-work-pc" not just "work-pc").
...
- **Where:** `phoenix-installer.cjs` (browser-based installer) and
...
## Phoenix Notify (Service Messaging)

- **Purpose:** Unified channel for Phoenix's own services to message the user.
  Scout, Dream, Pipeline, Memory all drop messages into a single Phoenix thread.
- **File:** `service/src/phoenix-notify.js`
- **API:**
  - `panNotify(service, subject, body, opts)` — post a message from a service
  - `panReply(userMessage)` — user replies in Phoenix thread → Cerebras Qwen responds
  - `ensurePanContact()` — idempotent boot setup (creates Phoenix contact + thread)
- **Severity:** `info` | `warning` | `critical`
- **Storage:** `chat_messages` table with service, severity, metadata fields
- **Service sign-offs:** Scout · 🔍, Dream · ✨, Pipeline · 🔬, Memory · 🧠
- **Where visible:** Comms panel (chat thread from Phoenix contact)
...
- **Purpose:** After every Claude session, evaluate if a novel reusable skill
  ...
  phoenix-local plugin marketplace.
...
- **Flow:**
  ...
  6. Logs to Phoenix server (best-effort)
...
| `/v2/comms` | Communications hub — threads, Phoenix messages | Built |
...
- **Structure:**
  ...
  - **Ring 2 sectors (8 major systems):**
    ...
    - Comms: Chat, Email, Phoenix Notify, Messaging Prefs
...
- **Purpose:** Thread-based messaging between the user, Phoenix, and other contacts.
  Also the channel through which Phoenix's own services (Scout, Dream, Pipeline)
  message the user.
...
- **Channels:**
  - `phoenix` — Phoenix system notifications (from phoenix-notify.js)
  - `dm` — Direct messages between users
- **Dashboard pages:**
  - `/v2/chat` — Chat interface (direct messaging)
  - `/v2/comms` — Communications hub (all threads, Phoenix thread, service messages)
  - `/v2/compose` — Compose and send messages
- **See also:** Phoenix Notify section (how services post into the ΠΑΛ thread)

---

## Email System

- **Purpose:** Send and receive email from within Phoenix.
...
- **Purpose:** Sync personal data between Phoenix instances (e.g. home PC ↔ laptop).
...
- **Integrates with:** Google Calendar (via GWS CLI), Phoenix task system.

---

## Call (`/v2/call`)

- **Purpose:** Audio/video calling UI within the Phoenix dashboard.
...
| `pan_restart` | Restart the Phoenix server (Craft swap or Carrier restart) |
...
### Router Tool (`phoenix`) — All Actions

Single `phoenix` tool dispatches to 20+ actions:
...
|-------------|---------|
| `phoenix://actions` | Markdown table of all router actions |
| `phoenix://alert-types` | Alert type definitions |
| `phoenix://services` | Service status JSON |
| `phoenix://stats` | Database statistics |
...
- Power: `👁 User (lvl 25)`
- User: `👤 owner (lvl 15)`
- Group: `🏢 Phoenix → Member (lvl 25)`

---

## Android self-update

Phoenix's Android app updates itself from the user's own Phoenix server — no Play Store, no manual sideload after the first install. Designed to be reusable for any future Phoenix-shipped Android app.
...
4. If `versionCode > BuildConfig.VERSION_CODE`:
   - Download to `cacheDir/updates/phoenix-${versionCode}.apk` (older APKs in that dir are deleted first).
...
- "Install unknown apps" must be granted once for Phoenix under Settings → Apps → Special access → Install unknown apps. If not granted, `UpdateChecker` deep-links the user there via `ACTION_MANAGE_UNKNOWN_APP_SOURCES`.

### FileProvider
- Authority: `dev.phoenix.app.fileprovider` (already declared for camera photos).
...
### Reusing for other Phoenix apps
The only app-specific bits are the `applicationId` and the FileProvider authority. To ship a second Phoenix-managed Android app:
...
- **pending_question** — what Phoenix (or commander) asked that's still unanswered
...
conceptually still "what Phoenix knows right now."
