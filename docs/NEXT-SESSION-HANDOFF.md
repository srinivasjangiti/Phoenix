...
Today we landed three router improvements that make Phoenix's voice replies feel grounded in the moment:

- **#744** — stopped Phoenix from running a memory search on every utterance; now it only fires when the user actually asks to recall something
...
- **#746** — router now reads Phoenix's recent thoughts (what intuition concluded in the last 5 min) so replies have continuity
...
- **#765** — Phoenix interject conversational triggers — confused/stuck/tangent/idle (maps to #NEW-4 in `docs/CONVERSATION-AND-INTERJECTION.md`)
...
- Engine: F5-TTS (already in Phoenix, cross-platform via Python, phrase cache → ~10ms common phrases, ~1-2s novel on GPU)
- Default device = server (local speakers via Python `sounddevice`); if `device` given, route via existing `speakSomewhere()`
- Surface: drop the widget idea; use the existing **Comms > Phoenix** chat thread as the conversation home — typing in that thread sends a message, Phoenix replies in text *and* speaks
- No toast banner (user vetoed)
- Phoenix chat thread becomes the unified log of: user→Phoenix, Phoenix→user reactive, Phoenix→user proactive interjections
...
- **(A) Bottom, near the mic button on the chat input bar** — feels natural ("mic = your voice in, speak = Phoenix's voice out")
...
My recommendation: **C, both.** The bottom one is the conversational trigger ("speak this reply"); the top one is the global "open Phoenix convo" shortcut. They have different purposes.
...
- **(C) New dashboard tab** — like opening a new Claude tab, but it's the Phoenix comms thread
...
**Q3 — When Phoenix speaks autonomously (intuition interjection), does it use the same chat thread?**

Right now, intuition interjections already go to the Phoenix chat thread via `phoenix-notify.js`. With our new design, those interjections would *also* get spoken aloud. So your Comms > Phoenix becomes the unified log of:
- Things you said to Phoenix
- Things Phoenix said in response
- Things Phoenix said proactively (interjections)
