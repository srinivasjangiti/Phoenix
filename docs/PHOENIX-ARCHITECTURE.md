...
>
> **Wiring between conversation, classifier, Phoenix's-Mind, and interjections lives in
...
Phoenix is **8 functional sections**. **Intuition is the cortex** — about half the
codebase by weight and the part that defines what Phoenix actually does. The
...
| # | Section | Role | Weight |
|---|---|---|---|
| 1 | **Intuition** | Perceive Commander's state, decide what (if anything) Phoenix should do | ~50% |
| 2 | **Memory** | Persistence that feeds Intuition with context, history, semantic facts | ~15% |
| 3 | **Capabilities** | The verb dictionary — what Phoenix *can* do (skills, MCP tools, modules) | ~8% |
...
| 7 | **Learning / Metabolism** | Phoenix improving itself — model hunting, dreaming, evolution, auto-skills | ~7% |
...
The dominant section. About half of Phoenix. Intuition perceives Commander's
...
| **Identity** | Who is the person in front of Phoenix? Face + voice + body + behavior fusion into a single cluster. | `webcam-watcher.js` (face), voice enrollment, identity events. Fragments only. | Partial — fusion missing |
| **Mind** | First-person reasoning trace — modeled on Claude's extended thinking. Every reasoning event writes one short sentence describing what Phoenix concluded. | `thoughts.js` + `phoenix_thoughts` table + MCP tool + dashboard panel | Live |
...
| **Action engine** | Picks the winning action, dispatches it through Comms+Devices, dedupes by candidate-identity (not by time) | — | **Planned** — currently no Phoenix-initiated dispatcher exists; only user-initiated `router.js` |
...
- **All channels, always.** When Phoenix acts, the message goes to every present
  channel (phone TTS, dashboard Phoenix thread, desktop TTS, client-PC toast,
...
|---|---|---|
| **Life Needs** (`phoenix_needs`, `phoenix_need_events`) | **user_id only** | Hydration, hunger, rest are facts about one human body. Same body in personal-org vs work-org has the same hydration — no org column. Cross-user separation is handled by user_id (different humans = different rows). |
...
| **Phoenix's Mind / thoughts** (`phoenix_thoughts`) | **user_id only** | First-person reasoning trace is one person's Phoenix thinking about one person. Org-level reasoning (a work-Phoenix reasoning about a whole company) is a separate future feature. |
...
| **Org-derived interjections** (future) | **org_id** | "Your standup is in 5min" — origin is the work calendar, delivery scoped to that org's Phoenix thread, members of that org see it. |

Federation between sovereign Phoenix nodes (your personal-Phoenix and a company's
work-Phoenix) is the layer above this — see `docs/FEDERATION.md` (planned) and
task #485. The above scoping holds *within one Phoenix server*. Cross-server
...
service/src/intuition/
  ...
  dispatch.js       # action delivery — calls phoenix-notify + TTS + push
```
...
What Phoenix *can do*, as a function library. A capability is a verb —
...
The plumbing that turns a "Phoenix should X" decision into actual work running on
...
| Client PCs | `phoenix-client/`, `client-manager.js`, `routes/client.js` |
...
| Channel | Files |
|---|---|
| TTS (out) | `tts.js`, `tts-worker.py`, Android TTS, `phoenix-voice.cs/.exe` |
| STT (in) | `dictate-vad.py`, `whisper-server.py`, Google streaming STT (phone) |
| Dashboard chat threads | `routes/chat.js`, Phoenix system thread |
...
| Service messaging | `phoenix-notify.js` (Scout/Dream/Pipeline → user) |
| Messaging preferences | `routes/messaging-prefs.js` |

---

## 7. Learning / Metabolism (Phoenix improving itself)

The self-loop. Phoenix finds better models for itself, dreams to reorganize its
own memory, evolves stale memory away, scans its own stack for issues,
auto-generates skills from novel sessions.

This is where Phoenix is the *subject*, not the user.
...
- **Governance** = what Phoenix is allowed to do on the user's behalf
...
| `terminal.js`, `super-carrier.js`, `carrier.js`, `platform.js`, `wezterm.js`, `playwright-bridge.js`, `discovery.js`, `hub-client.js`, `hub-crypto.js`, `client-manager.js`, `routes/client.js`, `routes/devices.js`, `screen-recorder.js`, `screen-buffer.js`, `remote-screen-watcher.js`, `phoenix-client/` | Devices |
| `tts.js`, `tts-worker.py`, `whisper-server.py`, `dictate-vad.py`, `audio-recorder.py`, `phoenix-voice.cs/.exe`, `phoenix-notify.js`, `routes/email.js`, `routes/chat.js`, `routes/messaging-prefs.js`, `email.js` | Comms |
...
- **The MCP server is the bridge** between Phoenix's capabilities and external
...
writes mostly to `phoenix_thoughts` or `intuition_snapshots`, it's Intuition.
