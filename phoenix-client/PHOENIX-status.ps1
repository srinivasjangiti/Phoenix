# PHOENIX-status.ps1 - Native status window for Phoenix Client.
#
# Opened from:
#   • Start Menu shortcut "Phoenix Status"
#   • Tray right-click → "Open Status..."
#
# Reads phoenix-status.json (same dir) and renders a compact WinForms window with
# every field the user might care about + action buttons. Auto-refreshes every
# 2 seconds while open.
#
# Run hidden console: `powershell.exe -WindowStyle Hidden -ExecutionPolicy Bypass -File PHOENIX-status.ps1`
...
if (-not (Test-Path $configPath) -and (Test-Path (Join-Path $scriptDir 'pan-client-config.json'))) {
...
$form.Text = 'Phoenix Client Status'
...
$titleLabel.Text = 'Phoenix Client'
...
function Update-Form {
  $s = Read-Status
  if (-not $s) {
    $stateLabel.Text = '* No status file - phoenix-client may not be running'
    $stateLabel.ForeColor = [System.Drawing.Color]::FromArgb(255, 239, 68, 68)
    $vDevice.Text = $env:COMPUTERNAME
    return
  }
  $titleLabel.Text = "Phoenix Client - $($s.device_name)"
