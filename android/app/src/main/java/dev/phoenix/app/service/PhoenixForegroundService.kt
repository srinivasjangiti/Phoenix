...
import dev.phoenix.app.MainActivity
import dev.phoenix.app.audio.FeedbackSounds
import dev.phoenix.app.camera.CameraCapture
...
import dev.phoenix.app.network.SyncManager
import dev.phoenix.app.network.dto.Action
import dev.phoenix.app.network.dto.AudioUpload
import dev.phoenix.app.network.dto.HistoryRequest
// GeminiBrain/MediaPipe REMOVED — all AI via server (Cerebras/Gemini through Tailscale)
import dev.phoenix.app.audio.VoiceCollector
import dev.phoenix.app.audio.BargeInMonitor
import dev.phoenix.app.stt.GoogleStreamingStt
import dev.phoenix.app.tts.TtsManager
...
@AndroidEntryPoint
class PhoenixForegroundService : Service() {
    ...
    companion object {
        ...
        // Phoenix deliberation surfaces — drives the native PanThinkingCard.
        ...
        // What Phoenix replied with on the previous turn.
    ...
    @Inject lateinit var serverClient: PhoenixServerClient
    ...
    @Inject lateinit var nanoVision: dev.phoenix.app.ai.NanoVision
    @Inject lateinit var sensorContext: dev.phoenix.app.sensor.SensorContext
    ...
    @Inject lateinit var localLlm: dev.phoenix.app.ai.LocalLlm
    ...
    // Persistent log — ships to Phoenix server via batched telemetry endpoint
    ...
    override fun onCreate() {
        super.onCreate()
        panLog("Phoenix service created")
        ...
        panLog("Build: versionCode=${dev.phoenix.app.BuildConfig.VERSION_CODE} versionName=${dev.phoenix.app.BuildConfig.VERSION_NAME} appId=${dev.phoenix.app.BuildConfig.APPLICATION_ID}")
        ...
        // Intuition glance — keep the ongoing notification showing Phoenix's live read (30s)
    ...
    // Polls Phoenix's live situational read and folds it into the ongoing notification,
    // so you can glance at your lock screen (no unlock) and see what Phoenix senses.
    ...
    private fun showPermissionNotification(perm: dev.phoenix.app.network.dto.PermissionPrompt) {
        ...
        val permChannel = NotificationChannel(
            permChannelId, "Phoenix Permissions",
        ...
        val allowIntent = Intent(this, PhoenixForegroundService::class.java).apply {
        ...
        val denyIntent = Intent(this, PhoenixForegroundService::class.java).apply {
            action = "PERMISSION_RESPOND"
            putExtra("response", "3")
            putExtra("perm_id", perm.id)
        }
    ...
    private fun ensureSlackChannel() {
        val channel = NotificationChannel(
            ...
            description = "Slack messages relayed through Phoenix — reply by voice or text"
            enableVibration(true)
        }
    ...
    private fun showSlackNotification(note: dev.phoenix.app.network.dto.SlackNotify) {
        ensureSlackChannel()
        ...
        val replyIntent = Intent(this, PhoenixForegroundService::class.java).apply {
            action = "SLACK_REPLY"
    ...
    private fun ensureAlertChannel() {
        val channel = NotificationChannel(
            ALERT_CHANNEL_ID, "Phoenix",
            NotificationManager.IMPORTANCE_HIGH
        ).apply {
            description = "Phoenix alerts (meeting reminders and other notifications)"
            enableVibration(true)
        }
        notificationManager?.createNotificationChannel(channel)
    }
    ...
    private fun showAlertNotification(note: dev.phoenix.app.network.dto.SlackNotify) {
        ensureAlertChannel()
    ...
    private fun onSpeech(text: String) {
        ...
        // Cooldown after TTS — ignore echo/residual audio for 2 seconds after Phoenix stops talking
        ...
        if ((lower.contains("mute") && !lower.contains("unmute")) || lower.contains("shut up") || lower.contains("be quiet")) {
            panLog("Phoenix muted by voice command")
            isMuted = true
            ...
            serviceScope.launch { dataRepository.addPanResponse("[Phoenix muted]") }
            return
        }
        if (lower.contains("unmute") || lower.contains("wake up") || lower.contains("start listening")) {
            panLog("Phoenix unmuted by voice command")
            isMuted = false
            ...
            serviceScope.launch { dataRepository.addUserMessage(text) }
            serviceScope.launch { dataRepository.addPanResponse("[Phoenix unmuted]") }
            return
        }
            ...
            if (lowerStripped.contains("calendar") || lowerStripped.contains("schedule") ||
                ...
                try {
                    ...
                    if (calResult != null) {
                        ...
                        addToHistory("User", text)
                        addToHistory("Phoenix", calResult)
            ...
            serviceScope.launch {
                try {
                    ...
                    if (response != null) {
                        mainHandler.post { panSpeak(response.response_text) }
                        addToHistory("User", text)
                        addToHistory("Phoenix", response.response_text)
                    } else {
        ...
        if (instantResponse != null) {
            ...
            addToHistory("User", text)
            addToHistory("Phoenix", instantResponse)
        ...
        serviceScope.launch {
            ...
            val localIntent = dev.phoenix.app.ai.LocalLlm.IntentResult(intent = "unknown", query = stripped, service = null, local = false, elapsedMs = 0)
                ...
                when (effectiveIntent.intent) {
                    ...
                    "query" -> {
                        ...
                        if (answerSource.equals("local", ignoreCase = true)) {
                            ...
                            if (answer.isNotBlank()) {
                                addToHistory("Phoenix", "$answer (${elapsed}ms local)")
                    ...
                    "terminal" -> {
                        ...
                        try {
                            ...
                            addToHistory("Phoenix", "$msg (${elapsed}ms)")
                            feedbackSounds.onCommandSent()
                            mainHandler.post { panSpeak(msg) }
                        } catch (e: Exception) {
            ...
            try {
                ...
                if (response != null) {
                    ...
                    if (responseText != "[AMBIENT]") {
                        addToHistory("Phoenix", "$responseText (${elapsed}ms)")
    ...
    private fun handleCameraCommand(userText: String) {
        ...
        // Check if camera is enabled in Phoenix sensor settings
        if (!sensorContext.cameraEnabled) {
            panLog("Camera is disabled in Phoenix sensor settings")
        ...
        serviceScope.launch {
            try {
                ...
                val hasCameraPermission = ContextCompat.checkSelfPermission(
                    this@PhoenixForegroundService, Manifest.permission.CAMERA
                ) == PackageManager.PERMISSION_GRANTED
                ...
                if (finalDescription != null && finalDescription.isNotBlank()) {
                    ...
                    addToHistory("Phoenix", finalDescription)
    ...
    private fun classifyLocally(text: String): String {
        ...
        // Explicit Phoenix address — always route
        if (lower.contains("hey phoenix") || lower.contains("hey pan") || lower.contains("hey pam")) {
            ...
            serviceScope.launch {
                try {
                    serverClient.api.respondPermission(
                        dev.phoenix.app.network.dto.PermissionRespondRequest(response, permId)
                    )
    ...
    private fun createNotificationChannel() {
        val channel = NotificationChannel(
            ...
            description = "Phoenix is always listening and remembering"
            setShowBadge(false)
        }
        notificationManager?.createNotificationChannel(channel)
    }
    ...
    private fun buildNotification(listening: Boolean, connected: Boolean): Notification {
        ...
        val toggleIntent = Intent(this, PhoenixForegroundService::class.java).apply {
            action = "TOGGLE_MIC"
        }
        ...
        val title = if (listening) "Phoenix Active" else "Phoenix Muted"
        ...
        val builder = NotificationCompat.Builder(this, Constants.NOTIFICATION_CHANNEL_ID)
            ...
            .addAction(0, "Stop", PendingIntent.getService(
                this, 2,
                Intent(this, PhoenixForegroundService::class.java).apply { action = "STOP_TTS" },
                PendingIntent.FLAG_UPDATE_CURRENT or PendingIntent.FLAG_IMMUTABLE
            ))
        // Expanded view = the full read Phoenix fused this tick + service status.
        intuitionFull?.let {
    ...
    private fun acquireWakeLock() {
        ...
        wakeLock = pm.newWakeLock(PowerManager.PARTIAL_WAKE_LOCK, "Phoenix::ServiceWakeLock")
        wakeLock?.acquire()
    }
