package dev.phoenix.app.ui.main
...
import dev.phoenix.app.network.dto.DeviceSensorConfig
import dev.phoenix.app.network.dto.IntuitionSnapshot
import dev.phoenix.app.network.dto.SensorAttachRequest
import dev.phoenix.app.network.dto.SensorUpdateRequest
import dev.phoenix.app.sensor.SensorContext
import dev.phoenix.app.service.PhoenixForegroundService
import dev.phoenix.app.stt.GoogleStreamingStt
import dev.phoenix.app.di.DeviceNameHolder
...
class MainViewModel @Inject constructor(
    ...
    val lastAction: StateFlow<String> = PhoenixForegroundService.lastAction
    val sttStatus: StateFlow<String> = PhoenixForegroundService.sttStatus

    // Phoenix deliberation surfaces — drive the PanThinkingCard in MainScreen.
    val isThinking: StateFlow<Boolean> = PhoenixForegroundService.isThinking
    val currentFiller: StateFlow<String> = PhoenixForegroundService.currentFiller
    val lastQueryText: StateFlow<String> = PhoenixForegroundService.lastQueryText
    val lastResponseText: StateFlow<String> = PhoenixForegroundService.lastResponseText
    val lastResponseMs: StateFlow<Long> = PhoenixForegroundService.lastResponseMs
    val lastFirstChunkMs: StateFlow<Long> = PhoenixForegroundService.lastFirstChunkMs

    val isMicEnabled: StateFlow<Boolean> = PhoenixForegroundService.micEnabled
    ...
    fun toggleMic() {
        ...
        val wasEnabled = PhoenixForegroundService.micEnabled.value
        if (wasEnabled) {
        ...
        val intent = android.content.Intent(application, PhoenixForegroundService::class.java).apply {
            action = "TOGGLE_MIC"
        }
    ...
    private fun syncHardwareToServerState(sensors: List<DeviceSensorConfig>) {
        for (s in sensors) {
            // Microphone sensor toggle = Phoenix permission, NOT Quick Mute
    ...
    fun toggleSensorEnabled(deviceId: Int, sensorId: String, enabled: Boolean) {
        // Sensor toggles control what Phoenix is ALLOWED to use (permissions)
    ...
    fun connectTailscale() {
        viewModelScope.launch {
            ...
            if (dev.phoenix.app.vpn.PhoenixVpn.isRunning() && remoteAccessManager.proxyPort.value > 0) {
                ...
                if (remoteAccessManager.status.value == "Connected") {
                    android.util.Log.d("Phoenix-VPN", "Already connected, proxy=${remoteAccessManager.proxyPort.value}")
                    return@launch
                }
                // Proxy exists but not working — stop and restart
                android.util.Log.d("Phoenix-VPN", "Proxy exists but not connected, restarting...")
                try { dev.phoenix.app.vpn.PhoenixVpn.disconnect(application) } catch (_: Exception) {}
            ...
            try {
                ...
                if (loginUrl != null) {
                    ...
                    for (i in 0 until 15) {
                        ...
                        android.util.Log.d("Phoenix-VPN", "Poll $i: status=${remoteAccessManager.status.value} proxy=$port")
                    ...
                    android.util.Log.d("Phoenix-VPN", "Final: status=${remoteAccessManager.status.value} proxy=$finalPort shouldUse=${remoteAccessManager.shouldUseTailscale}")
                }
            } catch (e: Exception) {
                android.util.Log.e("Phoenix-VPN", "connectTailscale failed: ${e.message}")
    ...
    fun toggleRemoteAccess(context: android.content.Context, enabled: Boolean) {
        viewModelScope.launch {
            if (enabled) {
                ...
                remoteAccessManager.setStatus("Connecting...")
                try {
                    val loginUrl = dev.phoenix.app.vpn.PhoenixVpn.connect(context)
                    if (loginUrl != null) {
                        dev.phoenix.app.vpn.PhoenixVpn.openLoginUrl(context, loginUrl)
                    }
                    remoteAccessManager.refreshFromVpn()
                } catch (e: Exception) {
                    remoteAccessManager.setStatus("Failed: ${e.message}")
                ...
                dev.phoenix.app.vpn.PhoenixVpn.disconnect(context)
