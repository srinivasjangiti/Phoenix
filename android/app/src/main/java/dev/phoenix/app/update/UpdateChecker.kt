package dev.phoenix.app.update
...
import dev.phoenix.app.BuildConfig
...
/**
 * Self-update flow — checks Phoenix server for a newer APK and triggers an install.
 *
 * Flow:
 ...
 *
 * Works for any future Phoenix-shipped Android app — the only thing app-specific
...
class UpdateChecker @Inject constructor(
    ...
    companion object {
        ...
        private const val FILE_PROVIDER_AUTHORITY = "dev.phoenix.app.fileprovider"
    ...
    private fun downloadApk(context: Context, url: String, versionCode: Int, expectedSize: Long): File? {
        ...
        dir.listFiles()?.forEach { f ->
            if ((f.name.startsWith("phoenix-") || f.name.startsWith("pan-")) && f.name.endsWith(".apk")) {
                runCatching { f.delete() }
            }
        }

        val file = File(dir, "phoenix-$versionCode.apk")
        val req = Request.Builder().url(url).build()
