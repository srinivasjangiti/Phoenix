# Phoenix 3 — Work-in-Progress Handoff

**Tab:** `dash-phoenix-1779102650923` (Phoenix 3)
...
## Resume into phoenix-3 with full transcript
...
```
~/.claude/projects/C--Users-owner-Desktop-Phoenix/
...
## What phoenix-3 was doing — by task

### ✅ DONE / shipped today

| Task | Status | Files |
|---|---|---|
| **Mic-input routing** (mirror of TTS speak-router) | shipped, loaded after craft swap @ 14:50 | `service/src/mic-router.js` (NEW, 212 LOC), `phoenix-client/phoenix-client.js` `+audio_capture` (+85 LOC), `service/src/server.js` `POST /api/v1/listen` + `GET /api/v1/listen/preview` (+25 LOC) |
| **T1 (#665) — schema additions for ownership** | DONE, live | `service/src/schema/ownership.js`: `devices.owner_user_id`, `devices.room`, `devices.display_name`, `identity_clusters.user_id`, `device_aliases.added_by_user_id`. All 5 devices auto-owned by owner. (Speculative tables `device_capabilities` + `device_preferences` were dropped — `smart-router.js` + `action_preferences` already exist.) |
| **T2 (#674) — phoenix-client capability poller** | `in_test`, staged on minipc, queued for desktop-pc | `phoenix-client/phoenix-client.js`: `staticCapabilities` + `liveCapabilities` + Proxy back-compat; `probeCapabilitiesWindows()` returns 21 live tags (BT, mic, speakers, display, gpu); 5-min refresh on WS-open/reconnect/HTTP poll. macOS/Linux stubs empty (no crash). |
...
| **minipc client restart via SSH** | DONE | PID 9580, WS connected, **26 caps** including paired iPhone via BT, AMD mic array. PowerShell scripts at `%USERPROFILE%\AppData\Local\Temp\phoenix-*.ps1`. |

### ⏸ AWAITING USER DECISION (this is where phoenix-3 stopped)

Last assistant message in phoenix-3 (15:27:38):
> **"T3 backend shipped and verified. Awaiting your call on T4 (mic arbitration) vs T6 (smart-router scoring)."**

Pick one to continue:

- **T4 — Mic arbitration**: every mic that hears an utterance reports RMS/SNR/match, server picks winner, losers suppress. Winner's device determines "where" for routing. Depends on live two-way WS (desktop-pc stale-WS bug below is a blocker).
- **T6 (#676) — Smart-router scoring extension**: `smart-router.js` already exists with `pickDevice()`, `NATURAL_DEVICE_HINTS`, `DEVICE_CAP_SCORES`. Only 3 small additions needed (not a rewrite). Independent of desktop-pc fix.

### 🐛 TWO OBSERVATIONS LEFT OPEN

1. **desktop-pc stale-WS**: dashboard shows `online=true`, but every WS-bound command falls to http-queue and times out. Either WS dropped without close-frame and DB wasn't updated, or queue routing is broken for desktop-pc. **Blocks T4** since mic arbitration needs live two-way comms.
2. **Two phoenix-client.js processes on minipc**: PID 10144 (relative path, parent = interactive PowerShell) + PID 8088 (absolute path at `%LOCALAPPDATA%\Phoenix\client\`, parent dead). Only one registers as minipc in DB. Killing 10144 may close someone's interactive PS window. Staged update only affects 8088.
...
| T11 | Installer prompt for `room` + `display_name` at setup; Phoenix asks in chat if device connects unnamed | Populates fields T6 needs |
...
**Carrier restart will kill phoenix-3's PTY** (PID 52460, 11h uptime — lives in Carrier, not Craft). The PTY is the bash shell hosting the adapter; restart spawns a fresh one. Transcript is preserved via the UUIDs above, so resume is `claude -r 1b07ef17…` after restart.
...
4. After reconnect: open phoenix-3, choose "resume 1b07ef17" or "resume 29a93581"
```
...
This file is the manual workaround until the open_tabs persistence bug stays fixed across restarts. If phoenix-3 looks empty again, read this file + `claude -r <uuid>` against the JSONLs listed at the top.
