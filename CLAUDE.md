...
Phoenix is a persistent AI operating system across all devices, projects, and conversations.

> **Architecture lives in [docs/PHOENIX-ARCHITECTURE.md](./docs/PHOENIX-ARCHITECTURE.md).**
> Phoenix is 8 functional sections — **Intuition is the cortex (~50% of the system)**;
...
> store at `lib/stores/terminal.svelte.js`, the `phoenix:*-update` WS event
...
> It maps the reactive loop (user → Phoenix), the proactive loop (Phoenix → user), and the
...
> **Dependency map: read [docs/PHOENIX-DEPENDENCY-MAP.md](./docs/PHOENIX-DEPENDENCY-MAP.md) before removing,
...
> phoenix-client WS lives on the Carrier (Craft's local client map is always empty).
...
graph TB
    ...
    subgraph Server["Phoenix Server (Node.js :7777)"]
...
- **Server**: Three-tier process hierarchy — Super-Carrier (7777, permanent) → Carrier (17760, restartable) → Craft (17700, hot-swappable). Unified router, SQLite/SQLCipher DB, project sync via .phoenix files, MCP server
...
- **Client devices**: phoenix-client.js installed on other PCs, registers via WS, receives commands. See `docs/MULTI-DEVICE-ROUTING.md`
- **Presence**: Webcam watcher (face ID, 30s) + Screen watcher (vision AI, 60s) → intuition.js context

### Current Projects (auto-detected from .phoenix files)
...
## API & Auth
- Phoenix server uses `claude -p` CLI (free, uses Claude Code subscription auth)
- OAuth token (sk-ant-oat01-*) does NOT work with Anthropic API directly
- For faster responses: add Anthropic API key for direct Haiku calls (~$2-5/month for Phoenix voice)
- Claude Code subscription ($100/month Max) covers all CLI usage

## Key Principle
Phoenix never forgets. Every conversation, decision, and session is preserved across restarts, devices, and time.

## User
Work autonomously — don't ask for permission, just do it.

## Session Continuity Rule
When a **fresh terminal session starts** (the very first message after `claude` launches), begin with a brief "Phoenix Remembers:" summary of recent topics from the "Recent Conversation" section below. This is ONLY for the first message of a fresh session — NEVER repeat it mid-conversation, NEVER repeat it on follow-up messages, and NEVER re-emit it after a PTY restart or context reload. If you've already said it once in this conversation, do not say it again.
...
|-----|------|----------|-----------|
| **Prod** | 7777 | `%LOCALAPPDATA%/Phoenix/data/` | Everything: terminal, steward, orphan reaper, device heartbeat, all services |
| **Dev** | 7781 | `%LOCALAPPDATA%/Phoenix/data-dev/` | Full copy of prod (terminal, dashboard, API, sensors, project sync). Skips only system-wide singletons: steward, orphan reaper, device heartbeat |
...
**CRITICAL**: Every `execSync()`, `exec()`, `execFile()`, `spawn()` call MUST include `windowsHide: true` in options. Without it, a visible black CMD window flashes on screen. Phoenix runs dozens of these per minute (health checks, process enumeration, taskkill) — missing `windowsHide` causes hundreds of CMD windows opening/closing.
...
| `service/dev-server.js` | Dev server launcher — sets PHOENIX_DEV=1, separate port/DB |
...
| `service/src/mcp-server.js` | MCP server — 8 tools + unified router (20+ actions) for Claude to interact with Phoenix |
...
| `service/src/client-manager.js` | Client WS server — handles phoenix-client connections, command queue, device registry |
...
| `service/src/phoenix-notify.js` | Service messaging — Scout/Dream/Pipeline → user via Phoenix chat thread |
...
| `service/src/routes/chat.js` | Chat system — threads, messages, Phoenix system channel |
| `service/src/routes/intuition.js` | Intuition engine — aggregates presence signals into voice router context |
| `service/src/thoughts.js` | Phoenix-Mind thought stream — first-person reasoning trace (writeThought/recentThoughts). Backs `phoenix_thoughts` MCP tool + dashboard panel. See docs/FEATURES.md |
...
| `service/installer/phoenix-installer.cjs` | Browser-based client installer with hardware model detection |
| `phoenix-client/phoenix-client.js` | Client agent — runs on remote PCs, receives + executes commands |
| `service/phoenix-loop.bat` | Windows respawn loop — restarts node on crash, stops on clean exit (code 0) |
| `service/public/mobile/index.html` | Phone dashboard — static HTML, no build step, served at /mobile/ |
| `service/dashboard/src/routes/terminal/+page.svelte` | Main dashboard UI (~9.4k lines). Center column (Terminal + Transcript + chat send) + Mail/Compose/Contacts + Perf. |
| `service/dashboard/src/lib/components/widgets/` | Dashboard panel widgets. |
| `service/dashboard/src/lib/components/modals/ImpersonatePanel.svelte` | Owner-only impersonation modal. |
| `service/dashboard/src/lib/stores/terminal.svelte.js` | Terminal-page shared reactive store. |

### Phone Dashboard Architecture
The phone opens the dashboard via **Android WebView** (not a browser — no address bar).
- **WebView source**: `android/app/src/main/java/dev/phoenix/app/ui/dashboard/DashboardScreen.kt`
...
- **Console logs**: `WebChromeClient` captures JS `console.log` → Android logcat as `PHOENIX-DASH JS:`
...
- **PHOENIX.bat**: Launches `phoenix-loop.bat` → spawns `node super-carrier.js`. Loop restarts on crash (exit ≠ 0), stops on clean exit
- **Sleep/wake**: After system sleep, old Craft may hold port 17700. Carrier cleanup + PHOENIX.bat exit-on-healthy prevents loops
...
> - The only thing that still warrants a heads-up: a `super-carrier` restart, because that kills port 7777 entirely. Currently no endpoint for this — only PHOENIX.bat relaunch. If you need it, name it explicitly and do the bat relaunch.
>
...
This is a fresh session for the "Phoenix" project.
...
- [#458 backlog] Tab list rebuild fragile across Craft swap — phoenix-4 tab disappeared after swap
...
- [#468 backlog] Phone: health-aware degradation banner — show 'Phoenix running on backup' when ollama/embeddings/whisper degraded
- [#469 backlog] Voice timeline panel in Kronos — per-prompt route, latency, response
- [#485 backlog] Phoenix federation: cross-node org communication (arch spike)
