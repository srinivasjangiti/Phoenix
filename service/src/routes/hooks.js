...
router.post('/PermissionRequest', async (req, res) => {
  ...
  try {
    ...
    console.log(`[Phoenix Hook] PermissionRequest: ${description.substring(0, 100)} — waiting for mobile response...`);
        ...
        (async () => {
          try {
            ...
            if (perm && perm.response) {
              // User responded!
              console.log(`[Phoenix Hook] PermissionRequest response: ${perm.response} (${Date.now() - startTime}ms)`);
              ...
              console.log(`[Phoenix Hook] PermissionRequest expired (removed from queue)`);
              settle('deny');
            } else if (Date.now() - startTime > MAX_WAIT) {
              // 115s hard timeout — deny
              console.log(`[Phoenix Hook] PermissionRequest timed out after ${MAX_WAIT}ms — denying`);
              try { clearPermission(permId); } catch {}
              settle('deny');
            }
          } catch (pollErr) {
            console.error(`[Phoenix Hook] PermissionRequest poll error (denying):`, pollErr?.message);
            settle('deny');
          } finally {
            isPolling = false;
          }
          ...
          console.error(`[Phoenix Hook] PermissionRequest IIFE escaped catch:`, escapeErr?.message);
          settle('deny');
        });
      }, POLL_INTERVAL);
    });
    ...
    console.error(`[Phoenix Hook] Error handling PermissionRequest:`, err.message);
    res.status(200).json({
      hookSpecificOutput: {
        hookEventName: 'PermissionRequest',
...
// Inject readable session context into CLAUDE.md between Phoenix-CONTEXT markers
...
async function injectSessionContext(cwd, orgId = 'org_personal', tabClaudeSessionIds = []) {
  try {
    ...
    if (startIdx === -1 || endIdx === -1) {
      startMarker = '<!-- PHOENIX-CONTEXT-START -->'; legacyStartMarker = '<!-- Phoenix-CONTEXT-START -->';
      endMarker = '<!-- PHOENIX-CONTEXT-END -->'; legacyEndMarker = '<!-- Phoenix-CONTEXT-END -->';
    ...
    const isNoise = (prompt) => {
      ...
      return p.startsWith('<task-')
          ...
          || p.startsWith('You are Phoenix') || p.startsWith('You are Phoenix')
          || p.startsWith('CURRENT STATE')
          || /^Phoenix Remembers:/i.test(p) || /^Π Remembers:/i.test(p)       // break the self-injection loop
          || p.toLowerCase().startsWith('phoenix remembers:') || p.toLowerCase().startsWith('pan remembers:')
          || p.length < 2;
    };
    ...
    // ── BUILD BRIEFING ────────────────────────────────────────────────────────
    let briefing = `## Phoenix Session Context\n\n`;
    briefing += `This is a fresh session for the "${project?.name || 'Phoenix'}" project.\n`;
    ...
    // Sanitize — strip any literal Phoenix-CONTEXT markers from injected content
    briefing = briefing.replace(/<!-- (?:PHOENIX|Phoenix)-CONTEXT-(START|END) -->/g, '');
    ...
    console.log(`[Phoenix Hook] Injected session context into ${claudeMdPath} (${newContent.length} chars, injection: ${briefing.length} chars)`);
    ...
    console.error(`[Phoenix Hook] Failed to inject session context:`, err.message);
  }
}
 ...
 *
 * This is the "let Phoenix narrate my Claude sessions" loop. Deliberately narrow:
 *
 ...
 *    this is safe to run: phoenix_interjections still records #564..#581 firing in
...
router.post('/internal/session-nudges', (req, res) => {
  try {
    ...
    // Surface as a high-urgency pan_alert so the dashboard can show "Phoenix
    ...
    console.log(`[Phoenix #769] session_nudge enqueued kind=${kind} session=${session_id.slice(0, 8)} id=${idNum}`);
    res.status(200).json({ ok: true, id: idNum });
  } catch (err) {
    console.error('[Phoenix #769] internal/session-nudges failed:', err.message);
...
router.post('/:eventType', (req, res) => {
  ...
  try {
    ...
    // (SessionStart hook often fails because Phoenix server isn't running yet when Claude starts)
    ...
    if (eventType === 'SessionStart') {
      ...
      // (source='resume'), the CLAUDE.md "Phoenix Remembers" preamble is suppressed
      ...
      // Phoenix Remembers path; 'compact'/'clear' are user-intentional and don't
      // need a banner.)
      if (payload.source === 'resume') {
        try {
          ...
          console.log(`[Phoenix Hook] SessionStart resume → pan_resumed broadcast (session=${sessionId.slice(0,8)}, last_topic="${lastTopic.slice(0,40)}")`);
        } catch (err) {
          console.warn('[Phoenix Hook] resume preamble failed:', err.message);
        }
      }
    }
    ...
    if (eventType === 'SessionEnd') {
      ...
      (async () => {
        try {
          ...
          console.error('[Phoenix Hook] SessionEnd buildSessionSummary failed:', err.message);
        }
        ...
        if (cwd) {
          injectSessionContext(cwd, orgIdForRecap, sessionId ? [sessionId] : []).catch(err => {
            console.error('[Phoenix Hook] SessionEnd context injection failed:', err.message);
          });
        }
      })();
      ...
      (async () => {
        try {
          ...
          if (recentEvents.length >= 2) {
            ...
            if (result.updated > 0) {
              console.log(`[Phoenix Hook] SessionEnd: auto-updated ${result.updated} tasks`);
            }
          }
        } catch (err) {
          console.error('[Phoenix Hook] SessionEnd task reconciliation failed:', err.message);
        }
      })();
    }
    ...
    // session id is different from the Phoenix PTY session id, but cwd matches.
    try {
      ...
      console.error('[Phoenix Hook] in-flight tracker failed:', e.message);
    }
    ...
    if (eventType === 'Stop' && payload.transcript_path && existsSync(payload.transcript_path)) {
      try {
        ...
        console.error('[Phoenix] Error extracting assistant messages:', err.message);
      }
    }

    // Auto-recap watermark — fire buildSessionSummary periodically during long
    // sessions so Phoenix Remembers stays fresh even if SessionEnd never fires
      ...
      try {
        ...
        if (shouldWrite) {
          ...
          buildSessionSummary(sessionId, cwd, orgIdForRecap).catch(err => {
            console.error('[Phoenix] watermark buildSessionSummary failed:', err.message);
          });
        }
      } catch (err) {
        console.error('[Phoenix] watermark check failed:', err.message);
      }
    }
    ...
    if (eventType === 'UserPromptSubmit') {
      try {
        ...
        if (pending) {
          ...
          console.log(`[Phoenix #769] UPS injecting nudge id=${pending.id} session=${sessionId.slice(0, 8)}`);
        ...
        console.error('[Phoenix #769] UPS nudge claim failed:', err.message);
      }
    }

    res.status(200).json({ ok: true });
  } catch (err) {
    console.error(`[Phoenix] Error handling ${eventType}:`, err.message);
