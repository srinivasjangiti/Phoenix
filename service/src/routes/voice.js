// Phoenix Voice Routes — Speaker enrollment, identification, voice print management
//
...
export function registerVoiceRoutes(app) {
  ...
  app.post('/api/v1/voice/enroll', async (req, res) => {
    ...
    if (!filePath && req.headers['content-type']?.startsWith('audio/')) {
      ...
      tempFile = join(tmpdir(), `phoenix-enroll-${randomBytes(8).toString('hex')}.wav`);
  ...
  app.post('/api/v1/voice/enroll-browser',
    ...
    async (req, res) => {
      ...
      const tmpFile = join(tmpdir(), `phoenix-enroll-${randomBytes(6).toString('hex')}.webm`);
      let ok = false;
      try {
  ...
  app.post('/api/v1/voice/transcribe-browser',
    ...
    async (req, res) => {
      ...
      const tmpFile = join(tmpdir(), `phoenix-stt-${randomBytes(6).toString('hex')}.${ext}`);
      try {
        writeFileSync(tmpFile, audioBuffer);
        if (!(await whisperAvailable())) {
          return res.status(503).json({ error: 'Voice server not running' });
