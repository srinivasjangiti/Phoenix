// Phoenix Client Routes — hub-side API for connected Phoenix clients.
//
// Endpoints:
...
router.get('/devices', (req, res) => {
  ...
  // trusted: 0 = pending, 1 = approved, -1 = denied, NULL = legacy (pre-phoenix-client)
  const dbDevices = all(
...
// Called by phoenix-client.js every 3s to receive queued commands (HTTP tunnel fallback).
...
router.get('/invite', async (req, res) => {
  ...
  const nodeCmd    = `node phoenix-client.js --hub ${wsUrl} --token ${token} --name ${name}`;
...
// Lightweight presence signal from phoenix-client devices. Called every 30s.
...
// Called by phoenix-client.js on first connect. Works through Cloudflare (no WebSocket needed).
...
router.post('/register', (req, res) => {
  ...
  try {
    ...
    console.log(`[Phoenix Clients] HTTP registration: ${deviceName} (${deviceId}) — pending approval`);
    res.json({ ok: true, status: 'pending', device_id: deviceId });
  } catch (e) {
    console.error('[Phoenix Clients] HTTP register error:', e.message);
    res.status(500).json({ ok: false, error: e.message });
  }
});

// ── GET /api/v1/client/status ─────────────────────────────────────────────────
// Polled by phoenix-client.js to check if hub owner has approved/denied the device.
