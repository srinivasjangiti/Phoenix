// Phoenix Chat routes — text messaging, contacts, WebRTC signaling
// Messages are E2E encrypted between Phoenix instances via the Hub relay.

import { Router } from 'express';
import crypto from 'crypto';
import { db } from '../db.js';
import { ensurePhoenixContact, phoenixReply, PHOENIX_THREAD_ID } from '../phoenix-notify.js';
...
export function ensureChatSchema(db) {
  db.exec(`
    ...
    CREATE TABLE IF NOT EXISTS contacts (
      ...
      phoenix_instance_id TEXT, pan_instance_id TEXT,                         -- their Phoenix instance ID (pan_xxxx)
  ...
  try {
    db.exec(`ALTER TABLE chat_messages ADD COLUMN channel TEXT DEFAULT 'phoenix'`);          // pan | email | slack | discord | telegram | whatsapp | sms
  } catch {}
  try {
  ...
  // Migration: federation columns on chat_threads (trust ladder + N-party Phoenix convos)
  // - org_id          → which org owns this thread (scoping for Phoenix↔Phoenix comms)
  // - scope_allowlist → JSON {"allowed":["calendar","work"], "denied":["health"]} per-thread allowlist
  //                     null = unrestricted (your own Phoenix, no federation gate)
  try {
  ...
  // Migration: Phoenix↔Phoenix body_types and intuition trace.
  ...
  // Seed the Phoenix system contact + thread (idempotent)
  ensurePhoenixContact();
}

// ─── Phoenix persona reply ───
// Called when user sends a message in the Phoenix thread — generates a context-aware reply
router.post('/phoenix-reply', async (req, res) => {
  const { message } = req.body || {};
  if (!message) return res.status(400).json({ error: 'message required' });
  try {
    const reply = await phoenixReply(message);
...
router.post('/threads/:threadId/messages', async (req, res) => {
  ...
  // NOTE: Phoenix auto-replies are triggered by the client via POST /api/v1/chat/phoenix-reply.
  // Do NOT also fire phoenixReply() here — that causes duplicate replies.
  // The /phoenix-reply endpoint is the single source of truth for Phoenix responses.

  // TODO: Send through Hub relay to recipient's Phoenix instance
...
// The actual media streams go peer-to-peer via WebRTC — Phoenix server never touches them.
...
// ─── Unified Inbox — merged Phoenix messages + emails ───

router.get('/unified-inbox', (req, res) => {
  const limit = parseInt(req.query.limit) || 50;
  const offset = parseInt(req.query.offset) || 0;
  const filter = req.query.filter || 'all'; // 'all' | 'phoenix' | 'email'

  const items = [];

  // Phoenix messages (incoming, sender != self)
  if (filter === 'all' || filter === 'phoenix' || filter === 'pan') {
...
router.post('/compose', async (req, res) => {
  ...
  if (!channel) return res.status(400).json({ error: 'channel required (phoenix, email, sms, etc.)' });
  ...
  try {
    if (channel === 'email') {
    ...
    } else if (channel === 'phoenix' || channel === 'pan') {
      // Send via Phoenix Hub relay
      if (!to_contact_id) return res.status(400).json({ error: 'Contact required for Phoenix messages' });
      ...
      db.prepare(`
        INSERT INTO chat_messages (id, thread_id, sender_id, body, body_type, message_type, channel, subject, created_at)
        VALUES (?, ?, 'self', ?, 'text', 'composed', 'phoenix', ?, ?)
      `).run(msgId, threadId, composedBody, subject || null, now);
      db.prepare('UPDATE chat_threads SET updated_at = ? WHERE id = ?').run(now, threadId);

      // TODO: Relay to recipient's Phoenix instance via Hub

      res.json({ ok: true, id: msgId, channel: 'phoenix', thread_id: threadId });
    ...
    console.error('[Phoenix] Compose send error:', e);
...
router.get('/contact-channels/:contactId', (req, res) => {
  ...
  const channels = [];
  if (contact.phoenix_instance_id || contact.pan_instance_id) channels.push({ id: 'phoenix', label: 'Phoenix', icon: '◆' });
...
router.get('/mail', (req, res) => {
  ...
  if (filter === 'all' || filter === 'phoenix' || filter === 'pan') {
    composed = db.prepare(`
      ...
      ${(filter === 'phoenix' || filter === 'pan') ? "AND (cm.channel = 'phoenix' OR cm.channel = 'pan')" : ''}
  ...
  const merged = [
    ...allComposed.map(m => ({
      id: m.id,
      type: m.channel === 'email' ? 'email' : 'phoenix',
