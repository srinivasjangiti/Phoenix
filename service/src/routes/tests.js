// Phoenix Test Runner — sequential dependency-chain test execution
...
//      - 'Phoenix remembers' must be visible (proves memory loaded + Phoenix started)
...
const RESTART_STATE_FILE = join(tmpdir(), 'phoenix-restart-test.json');
const SCREENSHOT_DIR = join(tmpdir(), 'phoenix-test-screenshots');
...
async function takeScreenshot(label) {
  ...
  try {
    ...
    if (result.ok && result.base64) {
      ...
      console.log(`[Phoenix Tests] Screenshot (Tauri): ${filepath}`);
      return filepath;
    }
    if (result.ok && result.path) {
      console.log(`[Phoenix Tests] Screenshot (Tauri path): ${result.path}`);
      return result.path;
    }
  } catch (err) {
    console.log(`[Phoenix Tests] Tauri screenshot failed: ${err.message}`);
  }
  ...
  try {
    ...
    if (result.ok && result.base64) {
      const imgBuf = Buffer.from(result.base64, 'base64');
      writeFileSync(filepath, imgBuf);
      console.log(`[Phoenix Tests] Screenshot (fallback): ${filepath}`);
      return filepath;
    }
  } catch (err) {
    console.log(`[Phoenix Tests] Screenshot fallback failed: ${err.message}`);
  }
...
function getPort() {
  return parseInt(process.env.PHOENIX_PORT) || 7777;
}
...
// ==================== Suite Definitions ====================
// EVERY suite depends on 'pan-remembers'. Nothing runs unless Phoenix starts and shows memory.
...
const suites = {
        ...
        run: async (ctx) => {
          ...
          try {
            await new Promise((resolve, reject) => {
              ...
              req.on('error', reject);
              req.write(JSON.stringify({ url: dashUrl, title: 'Phoenix Dev Terminal', label: 'dev' }));
              req.end();
            });
          } catch (err) {
  ...
  'pan-remembers': {
    name: 'Phoenix Remembers',
    description: "THE GATE: Start Phoenix terminal session, wait for 'Phoenix remembers' (proves memory + context injection works). NOTHING runs if this fails.",
    dependsOn: ['startup'],
    tests: [
      {
        id: 'phoenix-start-session', name: 'Start Phoenix terminal session',
        description: 'Connect WebSocket terminal, inject context briefing, launch Claude',
        run: async (ctx) => {
          const port = getPort();
          const cwd = '%USERPROFILE%\\Desktop\\Phoenix';
          const { ws, messages } = await connectWs({
            session: 'test-phoenix-remembers', project: 'Phoenix',
          ...
          return `Phoenix terminal session connected, Claude launched (context injected: ${briefingReady ? 'yes' : 'no'})`;
        }
      },
      {
        id: 'phoenix-remembers-wait', name: 'Wait for "Phoenix remembers"',
        ...
        run: async (ctx) => {
          ...
          if (!found) throw new Error(`"Phoenix remembers" not seen after ${elapsed}s — memory system broken`);
          return `Phoenix memory briefing appeared in ${elapsed}s`;
        }
      },
      {
        id: 'phoenix-remembers-screenshot', name: 'Phoenix Remembers (screenshot)',
        description: 'Screenshot proving "Phoenix remembers" is visible in the terminal',
        run: async (ctx) => {
          await wait(2000);
          const ssPath = await takeScreenshot('phoenix_remembers');
          if (ctx.ws) ctx.ws.close();
          if (!ssPath) throw new Error('Screenshot failed');
          return `VERIFY SCREENSHOT: ${ssPath} — "Phoenix remembers" must be visible in terminal output`;
        }
      }
    ]
  },
  ...
  'teams': {
    ...
    tests: [
      ...
      },
      {
        ...
        run: async (ctx) => {
          if (!ctx.testTeamId) throw new Error('No testTeamId');
          const port = parseInt((process.env.PHOENIX_DEV) ? '7781' : '7777');
          return new Promise((resolve, reject) => {
  ...
  'services': {
    ...
    description: 'Core Phoenix services are running: classifier, scout, dream, memory',
    dependsOn: ['phoenix-remembers'],
    tests: [
      {
    ...
    tests: [
      {
        ...
        run: async (ctx) => {
          const { ws, messages } = await connectWs({
            ...
            cwd: '%USERPROFILE%\\Desktop', cols: 80, rows: 24
          });
          ctx.ws = ws;
  ...
  'refresh': {
    name: 'Page Refresh',
    description: 'The #1 bug: opens Phoenix session, waits for Claude to fully start (Π remembers → ❯), sends message, simulates F5, verifies session and history survive.',
    dependsOn: ['phoenix-remembers'],
    tests: [
      {
        id: 'refresh-connect', name: 'Connect to existing Phoenix session',
        description: 'Connects to the dev-dash-phoenix/pan session where Claude is already running from Phoenix Remembers.',
        run: async (ctx) => {
          // Find the existing Phoenix session (created by the dashboard auto-launch)
          const sessions = await apiGet('/api/v1/terminal/sessions');
          const panSession = sessions.sessions?.find(s => s.id.includes('dev-dash-phoenix') || s.id.includes('dev-dash-pan') || s.project === 'Phoenix' || s.project === 'Phoenix');
          if (!panSession) throw new Error('No Phoenix session found — did Phoenix Remembers run?');
          ctx.sessionId = panSession.id;
          ctx.createdAt = panSession.createdAt;

          const { ws, messages } = await connectWs({
            session: panSession.id, project: 'Phoenix',
            cwd: '%USERPROFILE%\\Desktop\\Phoenix', cols: 120, rows: 30
          });
          ctx.ws = ws; ctx.messages = messages;
      ...
      },
      {
        ...
        run: async (ctx) => {
          const { ws, messages } = await connectWs({
            session: ctx.sessionId, project: 'Phoenix',
            cwd: '%USERPROFILE%\\Desktop\\Phoenix', cols: 120, rows: 30
          });
    ...
    tests: [
      {
        ...
        run: async (ctx) => {
          ...
          const connA = await connectWs({ session: sidA, project: 'Phoenix', cwd, cols: 80, rows: 24 });
          const connB = await connectWs({ session: sidB, project: 'Phoenix', cwd, cols: 80, rows: 24 });
      ...
      },
      {
        ...
        run: async (ctx) => {
          ...
          await apiPost('/hooks/PreToolUse', {
            ...
            cwd: '%USERPROFILE%/Personal Coding/Projects/Phoenix',
            tool_name: 'Bash',
    ...
    tests: [
      {
        ...
        run: async (ctx) => {
          ...
          if (withServices.length === 0) throw new Error('No projects have services defined in .phoenix or .pan');
    ...
    tests: [
      {
        ...
        description: 'Phoenix-CONTEXT-START and Phoenix-CONTEXT-END each appear exactly once',
        run: async () => {
          const fs = await import('fs');
          const claudeMd = fs.readFileSync('%USERPROFILE%/Personal Coding/Projects/Phoenix/CLAUDE.md', 'utf8');
          const starts = (claudeMd.match(/<!-- Phoenix-CONTEXT-START -->/g) || []).length;
          const ends = (claudeMd.match(/<!-- Phoenix-CONTEXT-END -->/g) || []).length;
          if (starts !== 1) throw new Error(`Phoenix-CONTEXT-START appears ${starts} times (expected 1)`);
          if (ends !== 1) throw new Error(`Phoenix-CONTEXT-END appears ${ends} times (expected 1)`);
          const startIdx = claudeMd.indexOf('<!-- Phoenix-CONTEXT-START -->');
          const endIdx = claudeMd.indexOf('<!-- Phoenix-CONTEXT-END -->');
      ...
      },
      {
        ...
        run: async () => {
          ...
          const hasState = claudeMd.includes('Phoenix State Document') || claudeMd.includes('What Works');
          if (!hasMemory && !hasState) throw new Error('No memory content injected — inject-context.cjs may have failed');
          const memoryDir = '%USERPROFILE%/.claude/projects/C--Users-owner-Desktop-Phoenix/memory';
      ...
      },
      {
        ...
        description: 'Content between markers must not contain literal Phoenix-CONTEXT markers',
        run: async () => {
          const fs = await import('fs');
          const claudeMd = fs.readFileSync('%USERPROFILE%/Personal Coding/Projects/Phoenix/CLAUDE.md', 'utf8');
          const start = claudeMd.indexOf('<!-- Phoenix-CONTEXT-START -->') + '<!-- Phoenix-CONTEXT-START -->'.length;
          const end = claudeMd.indexOf('<!-- Phoenix-CONTEXT-END -->');
          const between = claudeMd.substring(start, end);
          // Check for standalone markers (not as part of code descriptions)
          const strayStarts = (between.match(/^<!-- Phoenix-CONTEXT-START -->$/gm) || []).length;
          const strayEnds = (between.match(/^<!-- Phoenix-CONTEXT-END -->$/gm) || []).length;
      ...
      },
      {
        ...
        description: '.phoenix-state.md or .phoenix-briefing.md must have content',
        run: async () => {
          const fs = await import('fs');
          const briefing = '%USERPROFILE%/Personal Coding/Projects/Phoenix/.phoenix-briefing.md';
          const state = '%USERPROFILE%/Personal Coding/Projects/Phoenix/.phoenix-state.md';
          let source = 'none';
          let size = 0;
          if (fs.existsSync(briefing)) {
            const content = fs.readFileSync(briefing, 'utf8').trim();
            if (content.length > 0) { source = '.phoenix-briefing.md'; size = content.length; }
          }
          if (source === 'none' && fs.existsSync(state)) {
            const content = fs.readFileSync(state, 'utf8').trim();
            if (content.length > 0) { source = '.phoenix-state.md'; size = content.length; }
          }
          if (source === 'none') throw new Error('Both .phoenix-briefing.md and .phoenix-state.md are empty or missing');
      ...
      },
      {
        ...
        run: async () => {
          const fs = await import('fs');
          const before = fs.readFileSync('%USERPROFILE%/Personal Coding/Projects/Phoenix/CLAUDE.md', 'utf8').length;
          await apiPost('/api/v1/inject-context', { cwd: '%USERPROFILE%\\Desktop\\Phoenix' });
          const after = fs.readFileSync('%USERPROFILE%/Personal Coding/Projects/Phoenix/CLAUDE.md', 'utf8');
          const starts = (after.match(/<!-- Phoenix-CONTEXT-START -->/g) || []).length;
          const ends = (after.match(/<!-- Phoenix-CONTEXT-END -->/g) || []).length;
...
function getSuiteList() {
  return Object.entries(suites).map(([id, suite]) => ({
    ...
    group: suite.group || 'Phoenix Core',  // future: app-specific test groups
  }));
}
...
async function runTests(suiteId) {
  ...
  (async () => {
    ...
    console.log(`[Phoenix Tests] Run complete: ${totalPassed} passed, ${totalFailed} failed, ${totalSkipped} skipped. Screenshots: ${currentRun.screenshots.length}`);
  })();
...
async function resumeRestartTest() {
  const stateFile = existsSync(RESTART_STATE_FILE) ? RESTART_STATE_FILE : null;
  if (!stateFile) return;
  ...
  try {
    ...
    console.error('[Phoenix Tests] Failed to read restart state:', err.message);
    return;
  }

  console.log('[Phoenix Tests] Restart test state found — running post-restart verification...');
  ...
  console.log(`[Phoenix Tests] Restart verification complete: ${passed} passed, ${failed} failed`);
}
