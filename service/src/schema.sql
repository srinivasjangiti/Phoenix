...
-- Sensor definitions — all 22 Phoenix sensors (seeded on startup, not user-editable)
...
CREATE TABLE IF NOT EXISTS ai_usage (
    ...
    device_id TEXT,                   -- #465: e.g. "pixel-10-pro", "minipc", null for server-internal
...
-- Phoenix's stream of consciousness — first-person reasoning trace, modeled on
...
-- writes ONE short first-person sentence here. The dashboard's "Phoenix's Mind"
-- panel renders this stream verbatim, and Phoenix itself can read its own thoughts
-- back via the `phoenix_thoughts` MCP tool to remember "what was I doing 10 min ago?"
--
...
--     — NOT "Given the signals: commander=owner, screen=...")
--
...
CREATE TABLE IF NOT EXISTS phoenix_thoughts (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
...
CREATE INDEX IF NOT EXISTS idx_phoenix_thoughts_ts ON phoenix_thoughts(ts DESC);
CREATE INDEX IF NOT EXISTS idx_phoenix_thoughts_source ON phoenix_thoughts(source);
...
--                 | curiosity | safety
CREATE TABLE IF NOT EXISTS phoenix_needs (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    user_id TEXT NOT NULL,                          -- identity cluster id (e.g. 'owner')
...
CREATE INDEX IF NOT EXISTS idx_phoenix_needs_user ON phoenix_needs(user_id);
CREATE INDEX IF NOT EXISTS idx_phoenix_needs_need ON phoenix_needs(need_id);
...
CREATE TABLE IF NOT EXISTS phoenix_need_events (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    ts TEXT NOT NULL DEFAULT (datetime('now','localtime')),
...
CREATE INDEX IF NOT EXISTS idx_phoenix_need_events_ts ON phoenix_need_events(ts DESC);
CREATE INDEX IF NOT EXISTS idx_phoenix_need_events_user_need ON phoenix_need_events(user_id, need_id);

-- Delivered interjections — every time Phoenix actually speaks up. One row per
...
CREATE TABLE IF NOT EXISTS phoenix_interjections (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    created_at TEXT NOT NULL DEFAULT (datetime('now','localtime')),
    user_id TEXT NOT NULL,
...
CREATE INDEX IF NOT EXISTS idx_phoenix_interjections_user_created ON phoenix_interjections(user_id, created_at DESC);
CREATE INDEX IF NOT EXISTS idx_phoenix_interjections_key ON phoenix_interjections(action_key, created_at DESC);
CREATE INDEX IF NOT EXISTS idx_phoenix_interjections_status ON phoenix_interjections(status);
...
-- === SESSION SUMMARIES (recap source for Phoenix Remembers) ===
...
-- session as "Phoenix Remembers" and created a regurgitation loop (bug discussed
...
-- Typed thought records — the substrate Phoenix actually "thinks in".
...
-- the steps themselves are the durable substrate of Phoenix's mind.
--
-- Why: pre-#495 synthesis was prose-only LLM output with no pointers,
-- which is narration, not thought. With refs, Phoenix can pull up "the
-- screenshot I was looking at when I concluded X" the same way a human
-- pulls up an image from memory.
CREATE TABLE IF NOT EXISTS phoenix_reasoning_steps (
    id          INTEGER PRIMARY KEY AUTOINCREMENT,
...
CREATE INDEX IF NOT EXISTS idx_reasoning_ts        ON phoenix_reasoning_steps(ts DESC);
CREATE INDEX IF NOT EXISTS idx_reasoning_user_ts   ON phoenix_reasoning_steps(user_id, ts DESC);
CREATE INDEX IF NOT EXISTS idx_reasoning_cycle     ON phoenix_reasoning_steps(cycle_id);
CREATE INDEX IF NOT EXISTS idx_reasoning_subj_pred ON phoenix_reasoning_steps(subject, predicate);
CREATE INDEX IF NOT EXISTS idx_reasoning_kind      ON phoenix_reasoning_steps(kind);

-- FTS5 mirror over the free-text fields so we can do "what has Phoenix ever
-- thought about <topic>" queries without scanning. Triggers keep it in
-- sync with the base table.
CREATE VIRTUAL TABLE IF NOT EXISTS phoenix_reasoning_steps_fts USING fts5(
    subject, predicate, value,
    content='phoenix_reasoning_steps', content_rowid='id'
);
CREATE TRIGGER IF NOT EXISTS phoenix_reasoning_steps_ai AFTER INSERT ON phoenix_reasoning_steps BEGIN
    INSERT INTO phoenix_reasoning_steps_fts(rowid, subject, predicate, value)
    VALUES (new.id, new.subject, new.predicate, new.value);
END;
CREATE TRIGGER IF NOT EXISTS phoenix_reasoning_steps_ad AFTER DELETE ON phoenix_reasoning_steps BEGIN
    INSERT INTO phoenix_reasoning_steps_fts(phoenix_reasoning_steps_fts, rowid, subject, predicate, value)
    VALUES ('delete', old.id, old.subject, old.predicate, old.value);
END;
CREATE TRIGGER IF NOT EXISTS phoenix_reasoning_steps_au AFTER UPDATE ON phoenix_reasoning_steps BEGIN
    INSERT INTO phoenix_reasoning_steps_fts(phoenix_reasoning_steps_fts, rowid, subject, predicate, value)
    VALUES ('delete', old.id, old.subject, old.predicate, old.value);
    INSERT INTO phoenix_reasoning_steps_fts(rowid, subject, predicate, value)
    VALUES (new.id, new.subject, new.predicate, new.value);
...
CREATE TABLE IF NOT EXISTS phoenix_reasoning_cycles (
    id              INTEGER PRIMARY KEY AUTOINCREMENT,
...
CREATE INDEX IF NOT EXISTS idx_reasoning_cycles_ts      ON phoenix_reasoning_cycles(ts DESC);
CREATE INDEX IF NOT EXISTS idx_reasoning_cycles_user_ts ON phoenix_reasoning_cycles(user_id, ts DESC);
