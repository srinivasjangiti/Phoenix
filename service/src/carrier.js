...
//   Craft    — running Phoenix version (server.js with PHOENIX_CRAFT=1)
...
// Carrier has no DB — send Phoenix notifications via HTTP to the Craft
function phoenixNotify(service, subject, body, opts = {}) {
  ...
  try {
    const req = http.request({
      ...
      path: '/api/internal/phoenix-notify',
      method: 'POST',
...
const IS_UNDER_SUPER_CARRIER = !!(process.env.PHOENIX_UNDER_SUPER_CARRIER || process.env.PAN_UNDER_SUPER_CARRIER);
const CARRIER_PORT = IS_UNDER_SUPER_CARRIER
  ? (parseInt(process.env.PHOENIX_CARRIER_INTERNAL_PORT || process.env.PAN_CARRIER_INTERNAL_PORT) || 17760)
  : (parseInt(process.env.PHOENIX_PORT) || 7777);
...
// ==================== Client WebSocket (Phoenix Client devices) ====================
...
function spawnCraft(port, label = 'primary') {
  ...
  const craftEnv = {
    ...process.env,
    PHOENIX_CRAFT: '1',
    PHOENIX_CRAFT_PORT: String(port),
    PHOENIX_CRAFT_ID: String(id),
    PHOENIX_PORT: String(port),
    PHOENIX_CARRIER_PORT: String(CARRIER_PORT),
    };
...
async function runPipelineBenchmarks() {
  ...
  // Notify user via Phoenix contact thread
  try {
    const total = passedSuites.length + failedSuites.length;
    if (allPassed) {
      phoenixNotify('Pipeline · 🔬',
    ...
    console.warn('[Pipeline] phoenixNotify failed:', err.message);
  }
...
const carrierServer = http.createServer((req, res) => {
  ...
  // tabs, scrollback, Phoenix Remembers, etc.).
  ...
  // Full carrier restart — process.exit(1) so phoenix-loop.bat respawns us.
  ...
  if (url.pathname === '/api/carrier/restart' && req.method === 'POST') {
    ...
    try {
      ...
      const baseDir = join(process.env.LOCALAPPDATA || '', 'Phoenix');
      const markerDir = join(baseDir, isDev ? 'data-dev' : 'data');
    ...
    //    so phoenix-loop.bat respawns us with current disk code.
    setTimeout(() => {
  ...
  // Shutdown — kills Carrier and all children (used by PHOENIX.bat quit path)
  ...
  // Ready check — 200 only when Craft is healthy and proxying (used by PHOENIX.bat)
...
function confirmSwap() {
  ...
  if ((process.env.PHOENIX_CARRIER_POST_SWAP_BURST) === '1') {
  ...
  if ((process.env.PHOENIX_CARRIER_POST_SWAP_P1) === '1') {
    setTimeout(() => {
...
// FFmpeg thumbnail every 3 s for up to 30 s and sends F5 to the Phoenix window if
// the screen looks black, so the user never has to manually mash F5.

const BLACK_SNAP = join(tmpdir(), 'phoenix-swap-check.jpg');
...
async function sendF5ToPhoenixWindow() {
  try {
    await execFileAsync('powershell', ['-NoProfile', '-NonInteractive', '-Command', `
      $proc = Get-Process | Where-Object { ($_.MainWindowTitle -like 'Phoenix*') -and $_.MainWindowHandle -ne 0 } | Select-Object -First 1
      if ($proc) {
        Add-Type @'
...
'@
    ...
    console.log('[Carrier] Swap recovery: sent F5 to Phoenix window');
  } catch (e) {
    console.warn('[Carrier] Swap recovery: F5 send failed:', e.message);
  }
}
const sendF5ToPanWindow = sendF5ToPhoenixWindow;
...
async function boot() {
  ...
  try {
    ...
    if (featureEnabled('terminal_server')) {
      ...
      console.log(`[Carrier] Terminal/PTY WS server SKIPPED — profile ${process.env.PHOENIX_PROFILE || 'full'} (pipe mode still serves the phone over HTTP)`);
    }
  } catch (e) {
  ...
  // shows the blue "Phoenix restarting…" banner that only clears on a 30s safety
  ...
  try {
    const isDev = (process.env.PHOENIX_DEV) === '1';
    const phoenixMarkerPath = join(process.env.LOCALAPPDATA || '', 'Phoenix', isDev ? 'data-dev' : 'data', '.restart-pending');
    if (existsSync(phoenixMarkerPath)) {
      const ts = parseInt(readFileSync(phoenixMarkerPath, 'utf8'), 10);
      _restartPendingMarker = { path: phoenixMarkerPath, ts: Number.isFinite(ts) ? ts : Date.now() };
      try { unlinkSync(phoenixMarkerPath); } catch {}
  ...
  // Listen on main port — with retry + hard exit so phoenix-loop.bat can respawn cleanly
    ...
    carrierServer.once('error', async (err) => {
      if (err.code === 'EADDRINUSE') {
        if (listenAttempts < MAX_LISTEN_ATTEMPTS) {
          ...
          console.error(`[Carrier] ❌ Port ${CARRIER_PORT} still occupied after ${MAX_LISTEN_ATTEMPTS} attempts — exiting so phoenix-loop can respawn`);
          try { const _d = join(process.env.LOCALAPPDATA||'','Phoenix','data'); mkdirSync(_d,{recursive:true}); writeFileSync(join(_d,'.restart-pending'),Date.now().toString()); } catch {}
          process.exit(1);
        }
      } else {
  ...
  setInterval(async () => {
    try {
      ...
      if (activePid && activePid !== process.pid) {
        ...
        process.exit(0); // exit(0) = clean, don't trigger phoenix-loop respawn for this zombie
      }
