// Phoenix speak-router — task #496: cross-device TTS routing
//
...
// phoenix-client websocket/HTTP bridge — otherwise we fall through to the next rung.
//
...
export async function speakSomewhere(opts = {}) {
  ...
  for (let attempt = 0; attempt < 5; attempt++) {
    ...
    // Phone goes via push channel, not via phoenix-client.
    ...
    // phoenix-client desktop/server: tts_speak command via WS or HTTP queue.
    try {
      await sendToClient(choice.device, 'tts_speak', { text, voice, rate }, 8000);
      return { ok: true, device: choice.device, strategy: choice.strategy, via: 'phoenix-client' };
    } catch (e) {
