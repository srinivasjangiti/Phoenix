// Phoenix Client Manager — hub-side WebSocket server for phoenix-client connections.
//
...
export function createInviteToken(name = 'new-device', ttlMs = 24 * 60 * 60 * 1000) {
  const token = 'phoenix-' + crypto.randomBytes(16).toString('hex');
  const now = Date.now();
...
(function runMigrations() {
  const migrations = [
    ...
    // trusted: NULL = never seen as phoenix-client, 0 = pending approval, 1 = approved, -1 = denied
    ...
    // #497: how this device's phoenix-client is launched at startup.
...
function upsertDevice({ device_id, name, platform, capabilities, version, pending = false, tailscale_ip = null,
  ...
  if (existing) {
    // Never downgrade trust — if already approved (1) keep it; only set pending (0) on first-ever phoenix-client registration
...
export function approveDevice(device_id) {
  ...
  console.log(`[Phoenix Clients] Approved: ${device_id}`);
}
...
export function denyDevice(device_id, callerIp = null) {
  ...
  console.log(`[Phoenix Clients] Denied: ${device_id}`);
}
...
export function startClientServer(httpServer) {
  ...
  // SuperCarrier proxies existing phoenix-client connections; the new Carrier sends close(1001)
  // which triggers phoenix-client to reconnect and re-register. That round-trip takes ~500-1500ms.
  ...
  httpServer.on('upgrade', (request, socket, head) => {
    ...
    if (!isLocalhost && !isApproved && !isTokenValid(token)) {
      ...
      socket.destroy();
      console.warn(`[Phoenix Clients] Rejected connection: invalid token (device=${deviceId})`);
      return;
    }
  ...
  wss.on('connection', (ws, req) => {
    ...
    console.log(`[Phoenix Clients] New connection: ${deviceId}`);
    ...
    ws.on('close', () => {
      console.log(`[Phoenix Clients] Disconnected: ${deviceId}`);
    ...
    ws.on('error', (err) => {
      console.error(`[Phoenix Clients] WS error (${deviceId}):`, err.message);
    });
  ...
  console.log('[Phoenix Clients] Client WebSocket server ready on /ws/client');
  return wss;
}
...
function handleClientMessage(ws, deviceId, msg) {
  switch (msg.type) {
    ...
    case 'disconnect':
      console.log(`[Phoenix Clients] Clean disconnect: ${deviceId} (${msg.reason})`);
      clients.delete(deviceId);
      setDeviceOnline(deviceId, false);
      break;

    default:
      console.log(`[Phoenix Clients] Unknown message type: ${msg.type} from ${deviceId}`);
  }
}
...
function handleRegister(ws, deviceId, msg) {
  ...
  if (wasDenied) {
    ...
    if (now - rl.lastLog >= DENIED_LOG_INTERVAL_MS) {
      ...
      console.warn(`[Phoenix Clients] Blocked previously-denied device: ${deviceId}${suffix}`);
      rl.lastLog = now;
      rl.count = 0;
    }
  ...
  if (pending) {
    ...
    console.log(`[Phoenix Clients] Pending approval: ${msg.name || deviceId} (${msg.platform})`);
    ...
    console.log(`[Phoenix Clients] Registered: ${msg.name || deviceId} (${msg.platform}, v${msg.version})`);
...
function handleHeartbeat(deviceId, msg) {
  ...
  if (entry) {
    ...
    // phoenix-client.js getLocalClaudeStatus() for the shape.
  ...
  // Restart-history visibility: when phoenix-client has been trying to revive a
          ...
          createAlert({
            ...
            detail: JSON.stringify({
              ...
              hint: worst.step === 'probe_binary' ? 'Ollama not on PATH on the host machine'
                  ...
                  : 'Check the phoenix-client log for stderr',
            }),
          });
        }
      }
