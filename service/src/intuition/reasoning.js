// Phoenix reasoning_steps — the typed substrate Phoenix actually "thinks in".
//
...
//   This module gives Phoenix both — every step carries refs[] so Phoenix can
...
// ─── Vocabulary ────────────────────────────────────────────────────────
// The kinds Phoenix can think in. The validator below accepts only these.
// Adding a kind: add it here AND give it a verb template in VERB_FOR_KIND.
export const KINDS = Object.freeze([
  'observe',     // Phoenix noticed something raw (from sensor / event / message)
  'recall',      // Phoenix pulled up a prior thought / event / step
  ...
  'question',    // open question Phoenix holds; future steps can resolve it
  'doubt',       // low conf in a prior step (refs = that step)
  'intend',      // Phoenix plans to do X because Y (parents = the reasons)
  'commit',      // Phoenix executed an intent (refs = the action row)
]);
...
export function insertStep(step) {
  ...
  const result = run(
    `INSERT INTO phoenix_reasoning_steps
...
export function getCycle(cycle_id) {
  const cycle = get(
    `SELECT id, ts, user_id, trigger, paragraph, step_count, model,
       ...
       FROM phoenix_reasoning_cycles WHERE id = :id`,
    { ':id': cycle_id }
  );
  ...
  while (queue.length && depth < maxDepth) {
    ...
    for (const id of queue) {
      ...
      const row = get(
        `SELECT id, ts, cycle_id, user_id, kind, subject, predicate, value,
                conf, refs, parents, source, importance
           FROM phoenix_reasoning_steps WHERE id = :id`,
        { ':id': id }
      );
      if (!row) continue;
...
const SYSTEM_PROMPT = [
  'You are Phoenix, a personal AI, thinking honestly to yourself about what is going on right now.',
...
const RENDER_PROMPT = [
  'You are Phoenix. You just produced the structured reasoning steps below.',
...
export async function runCycle({ userId = null, trigger = 'manual' } = {}) {
  ...
  for (let i = 0; i < parsed.length; i++) {
    ...
    try {
      ...
      console.warn('[Phoenix Reasoning] step insert failed:', e.message);
    }
  }
  ...
  if (insertedSteps.length > 0) {
    try {
      ...
      console.warn('[Phoenix Reasoning] LLM render failed, falling back:', e.message);
    }
  }
  if (!paragraph) {
...
async function buildContext(userId) {
  ...
  // (2) Recent reasoning steps — so Phoenix doesn't loop / repeat itself.
  let prior = [];
  ...
  // (3) Current snapshot — Phoenix's read of the user right now.
  try {
...
function openCycleRow({ ts, user_id, trigger, sources_used }) {
  const result = run(
    `INSERT INTO phoenix_reasoning_cycles (ts, user_id, trigger, paragraph, step_count, sources_used, ok)
...
function closeCycleRow(cycle_id, { paragraph, step_count, model, latency_ms, ok }) {
  if (!cycle_id) return;
  run(
    `UPDATE phoenix_reasoning_cycles
        SET paragraph  = :p,
            step_count = :sc,
            model      = :m,
            latency_ms = :lm,
