// Phoenix's stream of consciousness — first-person reasoning trace.
//
// Modeled on Claude's extended-thinking blocks. Each reasoning event in Phoenix
...
// sentence describing what Phoenix concluded. The dashboard's "Phoenix's Mind" panel
// renders the stream verbatim, and Phoenix itself can read its own thoughts back
// via the `phoenix_thoughts` MCP tool.
//
// Why this exists:
//   • The old "What Phoenix's mind is busy with" feed showed raw `ai_usage` rows
 ...
 * string (`YYYY-MM-DD HH:MM:SS`). We use this because phoenix_thoughts.ts is
...
export function writeThought(source, thought, refs = null, importance = 0.5) {
  try {
    ...
    const result = run(
      `INSERT INTO phoenix_thoughts (source, thought, refs_json, importance)
...
export function recentThoughts(opts = {}) {
  try {
    ...
    if (opts.sinceMs && Number.isFinite(opts.sinceMs) && opts.sinceMs > 0) {
      // phoenix_thoughts.ts is stored as localtime via SQLite default
...
export function lastThoughtFor(source) {
  try {
    const rows = all(
      `SELECT id, ts, source, thought, refs_json, importance
       FROM phoenix_thoughts WHERE source = :s
...
export function recentThoughtMatches(source, thought, withinMs = 90_000) {
  try {
    ...
    const rows = all(
      `SELECT thought FROM phoenix_thoughts
