// Phoenix's Life Needs engine — Sims-style motive bars (0..100) per user.
//
// Model:
...
/**
 * Ensure a phoenix_needs row exists for (user, need). Returns the row.
...
function ensureRow(userId, needId) {
  ...
  let row = get(
    `SELECT * FROM phoenix_needs WHERE user_id = :u AND need_id = :n`,
    { ':u': userId, ':n': needId }
  );
  if (!row) {
    const def = NEED_DEFS[needId];
    run(
      `INSERT INTO phoenix_needs (user_id, need_id, level, weight, decay_rate, last_evaluated_at)
 ...
 *
 * Decay is APPLIED ONLY when there is at least one phoenix_need_events row for
...
export function evaluate(userId) {
  ...
  for (const needId of NEED_IDS) {
    ...
    // Persist decayed value so legacy consumers reading phoenix_needs.level stay
    // consistent with what we return here.
    run(
      `UPDATE phoenix_needs SET level = :lv, last_evaluated_at = :t
       WHERE user_id = :u AND need_id = :n`,
...
export function satisfy(userId, needId, opts = {}) {
  ...
  run(
    `UPDATE phoenix_needs
     SET level = :lv, last_satisfied_at = :t, last_evaluated_at = :t
     WHERE user_id = :u AND need_id = :n`,
    { ':lv': after, ':t': ts, ':u': userId, ':n': needId }
  );
  run(
    `INSERT INTO phoenix_need_events (user_id, need_id, kind, delta, level_after, source, refs_json, note)
...
export function manualSet(userId, needId, level, opts = {}) {
  ...
  run(
    `INSERT INTO phoenix_need_events (user_id, need_id, kind, delta, level_after, source, note)
...
export function adjustWeight(userId, needId, deltaWeight) {
  ...
  run(
    `UPDATE phoenix_needs SET weight = :w WHERE user_id = :u AND need_id = :n`,
...
export function recentEvents(userId, { limit = 20, needId = null } = {}) {
  ...
  if (needId) {
    return all(
      `SELECT * FROM phoenix_need_events
       WHERE user_id = :u AND need_id = :n
