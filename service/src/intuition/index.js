// Phoenix Intuition — the live situational state daemon.
//
// One source of truth for "what is Commander doing right now?" read by:
//   • Phoenix (voice/dispatcher) — resolves pronouns, picks agents
...
// Phoenix's *decision loop*, not a duplicate readout.
...
function fetchLiveSessions() {
  ...
  const req = http.get(`http://127.0.0.1:${CARRIER_PORT}/api/v1/terminal/sessions`, { timeout: 2000 }, (res) => {
    let body = '';
...
// ─── Known Phoenix feature keywords ───
...
// Uses keyword matching against known Phoenix concepts, falls back to project context.
function extractTopic(messages, projectName) {
  ...
  // Check for known Phoenix feature keywords (most specific wins)
  const hits = [];
...
function buildSnapshot(trigger = 'heartbeat') {
  ...
  // Recent terminal messages — what Commander is actually saying to Phoenix right now.
  ...
  // ─── Phoenix self-awareness: what are Phoenix's own systems doing? ───
  let stewardServices = [];
  ...
  try {
    // Recent Phoenix actions — what did Phoenix itself just do?
    const actions = all(`
  ...
  // If actively in terminal with Phoenix, that counts
  if (activeSessions.length > 0) social.add('Phoenix');
  ...
  if (social.size > 1) engagement = 'multi_conversation';   // Phoenix + others
  ...
  // NOT a medical device. These are Phoenix's assumptions about Commander's state.
  let mood = inferMood(recentTerminalMsgs, recentEvents, hour, now);

  // ─── WELLBEING — simple 3-state: ok / not_ok / emergency ───
  // NOT medical advice. Phoenix's assumption based on activity patterns and conversation.
  ...
  // Extract known Phoenix feature keywords from recent messages
  ...
  const predictions = [];
  if (social.size > 1) {
    const others = [...social].filter(s => s !== 'Phoenix');
...
// ─── Mood inference (NOT medical — Phoenix's assumptions) ───
...
// ─── Assumption inference (NOT medical — Phoenix's guesses ONLY) ───
// 3 states: ok, not_ok, emergency
// This is NOT a medical device. These are Phoenix's pattern-based guesses.
...
function buildClassifyPrompt(snap) {
  ...
  return `You are Intuition, Phoenix's situational awareness daemon. Given the following signals about Commander's current state, classify these axes. Reply ONLY with a JSON object, no explanation.
...
Classify:
{
  ...
  "need": "short phrase: what Commander needs from Phoenix right now, or null",
  "complexity": "one of: simple, moderate, complex, deep",
  "assumption": "one of: ok, not_ok, emergency",
  "assumption_detail": "one sentence: Phoenix's guess about Commander's wellbeing (NOT medical advice)"
}`;
}
...
async function classifyAxes(snap) {
  ...
  try {
    ...
    // Phoenix's-Mind thought — describe Phoenix's *decision*, not Commander's state.
    ...
    //     plus Phoenix's decision (hold / watch / consider interjecting).
    ...
    try {
      ...
      // need-decay model is unreliable: Phoenix literally can't see whether
      ...
      //   explicit list of needs Phoenix has a validated data source for. We
      //   previously tried "any non-decay phoenix_need_events row," but the
      ...
      try {
        if (sensorsStale) {
          ...
          for (const n of evald) {
            ...
            try {
              const recent = all(
                `SELECT status FROM phoenix_interjections
      ...
      if (key !== _lastInterjectionKey) {
        ...
        // delivers to all channels (Phoenix thread, dashboard WS, connected
...
// state (needs, hydration, rest) stays user-scoped — see docs/Phoenix-ARCHITECTURE.md.
//
...
//
// For single-user Phoenix: shows the commander + connected client devices.
// For multi-user Phoenix: shows all members' states rolled up.
//
// Tick cadence: same 60s as individual intuition (called from tickIntuition).

const ORG_INTUITION_FILE = path.join(
  process.env.LOCALAPPDATA || process.env.HOME || '.',
  'Phoenix', 'data', 'org-intuition.json'
);
...
async function buildOrgSnapshot(orgId = 'org_personal', trigger = 'heartbeat') {
  ...
  // ─── Connected devices (phoenix-client devices) ───
  let devices = [];
  try {
  ...
  // ─── Device presence (enriched from phoenix-client presence POSTs) ───
  try {
