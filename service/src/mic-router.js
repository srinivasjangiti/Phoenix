// Phoenix mic-router — symmetric to speak-router.js
//
...
export async function listenSomewhere(opts = {}) {
  ...
  for (let attempt = 0; attempt < 5; attempt++) {
    ...
    // phoenix-client desktop/server: audio_capture command via WS or HTTP queue.
    try {
      ...
      if (transcribe) {
        try {
          ...
          const port = process.env.PHOENIX_PORT || 7777;
