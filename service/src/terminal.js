// Phoenix Terminal — WebSocket-backed PTY sessions
...
function pipeSend(sessionId, userText) {
  ...
  if (!session) {
    console.error(`[Phoenix LLM] Session not found: ${sessionId}`);
    return false;
  }
  ...
  if (!session._llmAdapter) {
    ...
    if (provider === 'gemini') {
      ...
      console.log(`[Phoenix LLM] Created Gemini adapter for session ${sessionId}`);
    } else {
      // Default to Claude (built-in SDK)
      session._llmAdapter = new ClaudeAdapter(sessionId, session.cwd, onMessage, resumeId);
      console.log(`[Phoenix LLM] Created Claude adapter for session ${sessionId}`);
    }
    ...
    try {
      ...
      if (savedModel) {
        ...
        console.log(`[Phoenix LLM] Applied model preference: ${savedModel} (per-session=${!!session.model})`);
      }
    } catch {}
  }
  ...
  session._llmAdapter.send(userText)
    .then(() => {
      ...
      // registers a phoenixNotify hook so the phone hears the result).
    ...
    .catch((err) => {
      ...
      live.claudeRunning = false; // legacy compat
      console.error(`[Phoenix LLM] pipeSend error for ${sessionId}: ${err?.message}`);
...
function drainPipeCompleteCallbacks(session, err) {
  ...
  for (const cb of queue) {
    try { cb(err); }
    catch (e) { console.error('[Phoenix LLM] _onPipeComplete callback threw:', e?.message); }
  }
}
...
// drainPipeCompleteCallbacks() fires our hook which phoenixNotify()s the user
...
function delegateToPhoneToolsSession(userText) {
  ...
  session._onPipeComplete.push(async (err) => {
    try {
      if (err) {
        const { phoenixNotify } = await import('./phoenix-notify.js');
        phoenixNotify('Phoenix · ⚡',
      ...
      console.error('[Phoenix delegate] completion hook failed:', e?.message);
    }
  });
...
// adapter and resumes from JSONL. Safe to call without restarting Phoenix or killing the PTY.
// Use this when a session gets stuck (session.messages frozen, claudeRunning stuck, etc.).
function pipeResetAdapter(sessionId) {
  const session = sessions.get(sessionId);
  if (!session) {
    console.error(`[Phoenix LLM] pipeResetAdapter: session not found: ${sessionId}`);
    return false;
  }
  console.log(`[Phoenix LLM] Resetting adapter for session ${sessionId}`);
...
function pipeSetModel(sessionId, modelId) {
  ...
  if (session.pty && modelId) {
    ...
    console.log(`[Phoenix Terminal] Sent /model ${modelName} to PTY session ${sessionId}`);
    return true;
  }
...
function flushBroadcast(session) {
  ...
  for (const client of session.clients) {
    if (client.readyState === 1) {
      try { client.send(payload); }
        ...
        dead.push(client);
        console.warn(`[Phoenix #768] flushBroadcast send failed for session ${session.id || session.sessionId}: ${e?.message}`);
      }
    } else {
  ...
  if (dead.length > 0) {
    console.log(`[Phoenix #768] flushBroadcast pruned ${dead.length} dead client(s) from session ${session.id || session.sessionId}; ${session.clients.size} remain`);
  }
}

// ==================== Process Registry ====================
// Tracks ALL PIDs spawned by Phoenix: PTY shells, Claude CLI inside PTY, agent-sdk calls.
...
function registerProcess({ pid, type, sessionId, command, parentPid }) {
  ...
  console.log(`[Phoenix Registry] Registered ${type} PID ${pid} (session: ${sessionId || 'none'})`);
}
...
function deregisterProcess(pid, exitCode) {
  ...
  if (entry) {
    ...
    console.log(`[Phoenix Registry] Deregistered PID ${pid} (exit: ${exitCode})`);
...
async function discoverAndRegisterCliProcess(session, sessionId, bashPid) {
  try {
    ...
    console.error(`[Phoenix Terminal] Failed to discover CLI PID:`, e.message);
  }
}
...
function loadTokens() {
  try {
    if (existsSync(TOKENS_FILE)) {
      ...
      console.log(`[Phoenix Terminal] Loaded ${reconnectTokens.size} reconnect tokens`);
    }
  } catch (err) {
    console.error('[Phoenix Terminal] Failed to load reconnect tokens:', err.message);
  }
}
...
function saveTokens() {
  try {
    ...
    console.error('[Phoenix Terminal] Failed to save reconnect tokens:', err.message);
  }
}
...
function getInFlightTool(cwd, claudeSessionIds) {
  ...
  // Field evidence (2026-05-30): Phoenix tab whose reconnect token had cwd set
  // to WoE/Dev — bottom status bar persistently showed WoE Bash tool calls
  // while the actual conversation (top) was Phoenix. Removing the fallback fixes
  // the leak.
  //
...
async function startTerminalServer(httpServer) {
  ...
  // Single upgrade handler for all Phoenix WebSocket paths
  httpServer.on('upgrade', (request, socket, head) => {
    ...
    if (pathname === '/ws/terminal' || pathname === '/ws/terminal-dev') {
      ...
      import('./panel-broadcaster.js').then(m => {
        ...
        console.error('[Phoenix Terminal] panel-broadcaster import failed:', err.message);
  ...
  wss.on('connection', (ws, req) => {
    ...
    try { _handleTerminalConnection(ws, req); } catch (err) {
      console.error(`[Phoenix Terminal] FATAL connection handler error:`, err);
    ...
    for (const ws of wss.clients) {
      if (ws._isAlive === false) {
        ...
        if (ws._missedPongs >= 2) {
          console.warn(`[Phoenix #768] terminating dead WS after ${ws._missedPongs} missed pongs`);
  ...
  async function _handleTerminalConnection(ws, req) {
    ...
    const cwd = tokenEntry?.cwd || url.searchParams.get('cwd') || '%USERPROFILE%\\Desktop';
    ...
    if (!session && projectName && sessionId.startsWith('dash-') && !forceNew) {
      ...
      if (existingForProject) {
        ...
        if (existingId) {
          console.log(`[Phoenix Terminal] Dedup: redirecting new ${sessionId} → existing ${existingId} for project ${projectName}`);
          try {
    ...
    if (!session) {
      ...
      if (restored > 0) {
        console.log(`[Phoenix Terminal] Restored ${restored} log entries from prior session: ${sessionId}`);
      ...
      try {
        ...
        if (useConptyFalse) {
          console.log(`[Phoenix Terminal] PTY crash detected (STATUS_FATAL_APP_EXIT) — spawning with useConpty: false`);
        }
        ...
        if (crashEntry && crashEntry.count > 2 && (Date.now() - crashEntry.at < 5 * 60_000)) {
          console.warn(`[Phoenix Terminal] Rapid PTY crashes detected (${crashEntry.count} times) — waiting 5s before respawn`);
        ...
        try {
          ...
          if (provider === 'gemini') {
            setTimeout(() => {
              console.log(`[Phoenix Terminal] Auto-launching Gemini CLI in session ${sessionId}`);
              ptyProcess.write('gemini\r');
            }, 1000);
          }
        } catch (err) {
          console.error('[Phoenix Terminal] Auto-launch check failed:', err.message);
        }
        ...
        ptyProcess.onData((data) => {
          ...
          if (extracted.length > 0) {
            console.log(`[Phoenix Terminal] Extracted ${extracted.length} JSON events from PTY: ${extracted.map(e => e.type + (e.subtype ? ':' + e.subtype : '')).join(', ')}`);
          }
          for (const evt of extracted) {
            ...
            if (evt.type === 'system' && evt.subtype === 'init') {
              ...
              console.log(`[Phoenix Terminal] Stream init: claude_session=${evt.session_id} in ${sessionId}`);
              ...
              console.log(`[Phoenix Terminal] Stream result: turns=${evt.num_turns} cost=$${evt.total_cost_usd?.toFixed(4)} in ${sessionId}`, JSON.stringify(Object.keys(evt)));
        ...
        ptyProcess.onExit(({ exitCode }) => {
          ...
          try {
            ...
            console.error(`[Phoenix Terminal] Failed to log PtyExit:`, err.message);
          }
          console.log(`[Phoenix Terminal] PTY ${planned ? 'shutdown' : 'exited'}: ${sessionId} code=${exitCode} uptime=${Math.round(uptimeMs/1000)}s`);
          ...
          if (!session._plannedShutdown) {
            ...
            console.log(`[Phoenix Terminal] Planned shutdown — skipping crash alert for ${sessionId}`);
          }
        ...
        if (cwd) {
          try {
            ...
            console.log(`[Phoenix Terminal] Pre-injected session context for ${projectName || sessionId} (tabIds: ${tabIds.length})`);
          } catch (err) {
            console.error(`[Phoenix Terminal] Context injection failed:`, err.message);
          }
        }

        console.log(`[Phoenix Terminal] New session: ${sessionId} (${projectName || 'shell'}) in ${cwd}`);
        ...
        console.error(`[Phoenix Terminal] Failed to spawn PTY:`, err.message);
    ...
    if (claudeSessionIdsFromToken.length > 0 && session) {
      ...
      console.log(`[Phoenix Terminal] Restored Claude session IDs from token: ${claudeSessionIdsFromToken.join(', ')}`);
    }
    ...
    if (tokenExpired) {
      console.log(`[Phoenix Terminal] Reconnect token expired/invalid for session ${sessionId} — falling back to fresh params`);
    }
  ...
  console.log(`[Phoenix Terminal] Server-side rendered terminal ready at /ws/terminal`);
}
...
function broadcastRenderedScreen(session, singleClient) {
  ...
  if (tDone - t0 > 10) {
    console.log(`[Phoenix Terminal] Slow frame: render=${(tRender-t0).toFixed(1)}ms total=${(tDone-t0).toFixed(1)}ms size=${msg.length}`);
  }
...
async function killAllSessions() {
  ...
  for (const [id, session] of sessions) {
    ...
    try {
      ...
      console.error(`[Phoenix Terminal] Failed to flush log for ${id}:`, err.message);
    }
    try {
...
function logPtyInput(sessionId, text, source, extra = {}) {
  try {
    ...
    if (source === 'unknown') {
      console.warn(`[Phoenix PTY] UNKNOWN-source input → ${sessionId}: ${JSON.stringify(t.slice(0, 80))}`);
    }
  } catch (e) {
    // Never let logging break a PTY write
    console.warn(`[Phoenix PTY] logPtyInput failed: ${e.message}`);
  }
}
...
function sendToSession(sessionId, text, label, source = 'unknown') {
  console.log(`[Phoenix Terminal] sendToSession(${sessionId}, ${JSON.stringify(text)}, source=${source}) bytes: ${[...text].map(c => c.charCodeAt(0).toString(16)).join(' ')}`);
  let session;
  if (sessionId) {
...
function respondToPermission(id, response) {
  console.log(`[Phoenix Perm] respondToPermission called: id=${id} (type=${typeof id}), response=${response}`);
  console.log(`[Phoenix Perm] Pending permissions: ${pendingPermissions.map(p => `${p.id}(type=${typeof p.id})`).join(', ')}`);
  ...
  if (perm) {
    ...
    console.log(`[Phoenix Perm] Set response=${response} on perm ${perm.id}`);
    return true;
  }
  console.log(`[Phoenix Perm] Permission ${id} NOT FOUND in pending list`);
  return false;
}
...
function createPipeSession(sessionId, { cwd = null, projectName = '' } = {}) {
  ...
  const resolvedCwd = cwd || process.env.USERPROFILE || '%USERPROFILE%\\Desktop';
  ...
  sessions.set(sessionId, session);
  console.log(`[Phoenix Terminal] Created pipe session: ${sessionId} cwd=${resolvedCwd}`);
  return session;
}
...
// transcript to `~/.claude/projects/<cwd-slug>/<uuid>.jsonl`. Phoenix needs to
...
function cwdToProjectSlug(cwd) {
  ...
  // Observed: `%USERPROFILE%\Desktop\Phoenix` → `C--Users-owner-Desktop-Phoenix`
