...
//
//   • current topic              ("Phoenix endpointing design")
//   • phase                       ("discussing options" | "wrapping up" | "command")
//   • pending question Phoenix asked  ("which option A/B/C/D?")
...
// clocks. But conv-state IS conceptually part of "what Phoenix knows right now"
...
async function _distill(orgId) {
  ...
  const prompt = `You are Phoenix's Conversation State distiller. Reading the recent voice/chat utterances plus ambient context, output a JSON object that captures the live state of the conversation. The router will read this on every turn — it must be terse and accurate.
...
Output strict JSON (no prose, no markdown fences) with these fields:
{
  ...
  "pending_question": "if Phoenix (or commander) asked something not yet answered, the question — else null",
