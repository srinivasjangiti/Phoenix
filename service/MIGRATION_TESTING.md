...
```
[Phoenix MemorySearch] migration: dimension mismatch on first-tracked boot — dropping N stale rows (incompatible with qwen3-embedding@1024)
[Phoenix MemorySearch] backfill starting: 0/TOTAL indexed, TOTAL to embed
[Phoenix MemorySearch] backfill: +200/TOTAL (200/TOTAL total)   ← every 200 events
...
[Phoenix MemorySearch] backfill complete: +N embeddings (N/TOTAL total)
```
...
```
[Phoenix MemorySearch] migration: embedding config changed — qwen3-embedding@1024 → test-model-768@768. Dropping N rows and rebuilding.
[Phoenix MemorySearch] backfill starting: ...
```
...
|---|---|
| `[Phoenix MemorySearch] vec query failed: Dimension mismatch` on every search | Migration didn't run or ran against a different DB handle than the query |
