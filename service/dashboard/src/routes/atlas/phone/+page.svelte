<script>
	...
	// Distinct from the main Phoenix Atlas (radial server map). The phone is a linear
	...
	const NODES = [
		...
		{ id: 'thinking', label: 'PanThinkingCard',    x: 700,  y: 340, kind: 'ui',
		  detect: m => m.startsWith('Stream (first=') ? 'ok' : null,
		  desc: 'Native Compose card showing what Phoenix is thinking + last response time. Filler ("hold on...") shown if first chunk takes >1.5s.' },
		{ id: 'service',  label: 'Phoenix Service',        x: 60,   y: 80,  kind: 'core',
		  detect: m => m === 'Phoenix service created' ? 'ok' : null,
		...
		<aside class="detail">
			...
			{:else}
				<div class="hint">
					...
					<h3>Quick triage</h3>
					<ul class="triage">
						<li><strong>"Phoenix cut itself off"</strong> → check <em>BargeIn</em> node — red means false self-interrupt</li>
