<script>
	...
	function buildGraph(svcResp, atlasResp, statsResp, projResp) {
		...
		// ===== RING 0: CENTER — Phoenix Core =====
		add('phoenix-server', 'Phoenix Server', 'core', 'up',
		...
		add('steward', 'Steward', 'service', svcStatus(am, 'phoenix-server'), 'Health monitor', 'core', r1pos[2].x, r1pos[2].y, 'Monitors and restarts all services.');
		add('tauri', 'Tauri Shell', 'ui', 'up', 'Port 7790', 'core', r1pos[3].x, r1pos[3].y, 'Desktop shell.');

		edges.push({ from: 'phoenix-server', to: 'database', color: EC.core });
		edges.push({ from: 'phoenix-server', to: 'dashboard', color: EC.core });
		edges.push({ from: 'phoenix-server', to: 'steward', color: EC.core });
		edges.push({ from: 'phoenix-server', to: 'tauri', color: EC.core });
		...
		const procItems = [
			...
			['dream', 'Dream Cycle', svcStatus(am, 'dream'), svcDetail(am, 'dream'), 'Rewrites .phoenix-state.md every 6h.'],
		...
		// This is "Phoenix never forgets" — the loop that makes memory persistent.
		...
		// The complete voice loop: Phone Mic → Google STT (cloud) → Phoenix Server →
		...
		edges.push({ from: 'google-stt',   to: 'phoenix-server',   color: voiceColor, label: 'transcript' });
		edges.push({ from: 'phoenix-server',   to: 'voice-router', color: voiceColor });
		...
		if (uniqueDevices.length > 0) {
			...
			uniqueDevices.forEach((d, i) => {
				...
				edges.push({ from: 'phoenix-server', to: `dev-${d.name}`, color: EC.dev });
			});
		}
...
>
	<div class="atlas-topbar">
		<span class="atlas-title">Phoenix Atlas</span>
				...
				{#each atlasData.nodes as node}
					...
					>
						{#if node.id === 'phoenix-server'}
							...
							/>
							<text y="-8" fill="#89b4fa" font-size="14" font-weight="700" text-anchor="middle" font-family="sans-serif">PHX</text>
							<text y="12" fill="#cdd6f4" font-size="10" font-weight="600" text-anchor="middle">Phoenix Server</text>
