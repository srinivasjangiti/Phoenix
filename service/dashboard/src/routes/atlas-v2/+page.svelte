<script>
	...
	// ==================== CODENAME MAP ====================
	const CODENAMES = {
		'phoenix-server': { code: 'Nexus', tech: 'Phoenix Server' },
	...
	function getNodes() {
		...
		// Sun
		nodes.push({ id: 'phoenix-server', ring: 0, x: CX, y: CY, type: 'core', sector: 'core', isPlanet: false });
		...
		for (const m of NEXUS_MOONS) {
			...
			nodes.push({ ...m, ring: 0.5, sector: 'core', parentPlanet: 'phoenix-server', x: pos.x, y: pos.y, isPlanet: false });
		}
		...
		for (let pass = 0; pass < 4; pass++) {
			for (let i = 1; i < nodes.length; i++) {
				if (nodes[i].id === 'phoenix-server' || customPositions[nodes[i].id]) continue;
				for (let j = i + 1; j < nodes.length; j++) {
					if (nodes[j].id === 'phoenix-server' || customPositions[nodes[j].id]) continue;
	...
	function getEdges(nodes) {
		...
		// Sun → Nexus moons
		for (const m of NEXUS_MOONS) e('phoenix-server', m.id, '#89dceb50');

		// Sun → planets (gravitational lines)
		e('phoenix-server', 'database', '#fab38740');
		e('phoenix-server', 'steward', '#a6e3a130');
		e('phoenix-server', 'memory-hub', '#cba6f730');
		e('phoenix-server', 'dream', '#f9e2af25');
		e('phoenix-server', 'autodev', '#f38ba820');
	...
	function getRealtimeValue(id) {
		const s = svc(id);
		switch (id) {
			case 'phoenix-server': return stats ? `${(stats.total_events / 1000).toFixed(1)}K Events` : null;
...
>
	<div class="topbar">
		<span class="title">Phoenix Atlas <span class="v2">v2</span></span>
		<div class="controls">
				...
				{#each allNodes as node}
					...
					>
						{#if node.id === 'phoenix-server'}
							...
							<text y="-18" fill="#89b4fa" font-size="20" font-weight="700" text-anchor="middle" font-family="sans-serif">PHX</text>
