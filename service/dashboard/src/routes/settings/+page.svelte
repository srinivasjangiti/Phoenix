<script>
	...
	function updateBrandingLogo(value) {
		brandingLogo = value;
		localStorage.setItem('phoenix_branding_logo', value);
		fireBrandingChange();
	}
	function updateBrandingImage(dataUrl) {
		brandingImage = dataUrl;
		if (dataUrl) {
			localStorage.setItem('phoenix_branding_image', dataUrl);
		} else {
			localStorage.removeItem('phoenix_branding_image');
		}
		fireBrandingChange();
	}
	function fireBrandingChange() {
		window.dispatchEvent(new CustomEvent('phoenix-branding-changed', { detail: { logo: brandingLogo, image: brandingImage } }));
	}
	...
	async function restartServer() {
		...
		// See FEATURES.md → "Settings → Restart Phoenix" for the full spec.
		flash('Restarting Phoenix (carrier + craft)...');
		let startedOk = false;
		try {
			...
			if (r.status === 409) {
				...
				const proceed = confirm(`Phoenix is not fully healthy (failed: ${failed}). Force restart anyway?`);
		...
		flash('Waiting for Phoenix to come back...');
		let attempts = 0;
		const poll = setInterval(async () => {
			...
			try {
				...
				if (resp.ok) {
					clearInterval(poll);
					flash('Phoenix back online ✓');
					setTimeout(() => location.reload(), 800);
				}
			} catch {}
			if (attempts >= 60) {
				clearInterval(poll);
				flash('Phoenix did not come back after 60s — check service window');
			}
		}, 1000);
	}
	...
	async function saveTerminalAI() {
		...
		try {
			...
			// Signal all open terminal tabs to clear their launch guard so Phoenix Remembers re-fires
...
<div class="settings-layout">
	...
	<div class="panel">
		...
		{#if activeTab === 'general'}
			...
			<section class="section">
				...
				<div style="margin-top:10px">
					<button class="btn warn" onclick={restartServer} title="Full Phoenix restart — kills carrier, respawns it. Use when carrier.js / perf stages / engine code changed. For Craft-only changes use the Lifeboat widget instead.">Restart Phoenix</button>
				</div>
			</section>
			...
			<section class="section">
				...
				<p class="hint">When enabled, Phoenix adds extra confirmations before destructive actions and explains technical concepts in plain language.</p>
			</section>

			<section class="section">
				<h3>Personality</h3>
				<p class="hint">Describe how Phoenix should talk. Examples: "Talk like Morgan Freeman — calm, wise, measured", "Be sarcastic and witty like Tony Stark", "Speak with a British butler's formality", "Be enthusiastic and energetic like a sports commentator"</p>
				<textarea
					class="personality-input"
					value={settings.personality || ''}
					placeholder="Leave empty for default Phoenix personality..."
			...
			<section class="section">
				...
				<div class="row">
					...
					<input type="text" class="term-input" value={brandingLogo} oninput={(e) => updateBrandingLogo(e.target.value)} placeholder="Phoenix" />
				</div>
				<div class="row">
					...
					<div style="display:flex; align-items:center; gap:12px; background:#0a0a0f; padding:12px 20px; border-radius:8px;">
						...
						{:else}
							<span style="font-family: serif; font-size: 32px; font-weight: 700; color: #89b4fa;">{brandingLogo || 'Phoenix'}</span>
						{/if}
					</div>
				</div>
			</section>
			...
			<section class="section">
				...
				<div class="row">
					...
					<input type="text" class="term-input" value={termSettings.username} oninput={(e) => updateTermSetting('phoenix_username', e.target.value)} placeholder="Your name" />
				</div>
				<div class="row">
		...
		{#if activeTab === 'network'}
			...
			<section class="section">
				<h3>Remote Access</h3>
				<p class="hint">Access Phoenix from anywhere, not just your home WiFi. One-click setup -- no technical knowledge needed.</p>
				...
				{#if tailscaleStatus && !tailscaleStatus.installed}
					<div class="warn-box">
						...
						<p class="hint">Enable remote access so you can reach Phoenix from anywhere -- your phone on cellular, a laptop at a coffee shop, or another network entirely.</p>
			...
			<section class="section">
				...
				<p class="hint" style="margin-top:4px">When set, new phones auto-join your Tailscale network after signing into Phoenix. No separate Tailscale login needed.</p>
			</section>
		...
		{#if activeTab === 'treasury'}
			...
			<section class="section">
				<h3>Data Dividend</h3>
				<p class="hint">Stake your anonymized data to help improve AI. You earn Phoenix tokens when your data is purchased. All personal information is stripped on-device before anything leaves your machine.</p>
				<div class="toggle-row">
					<label class="toggle">
				...
				<p class="small muted" style="margin-top:4px">When enabled, anonymized data is contributed to the Phoenix network. You earn rewards proportional to your contribution.</p>
			</section>
				...
				{#if treasurySettings.wallet}
					...
					<div class="stat-row" style="margin-top:12px">
						<div class="stat-card">
							...
							<div class="stat-label">Phoenix Balance</div>
						</div>
						<div class="stat-card">
...
<style>
	...
	.theme-card.active {
		border-color: var(--phoenix-accent, #89b4fa);
		background: var(--phoenix-surface2, #1a1a25);
		box-shadow: 0 0 12px var(--phoenix-glow, rgba(137,180,250,0.2));
	}
