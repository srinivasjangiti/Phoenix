<script>
	...
	let testGroup = $state('Phoenix Core');
	...
	async function createTab(sessionId, projectName, cwd, projectId, isReconnect) {
		...
		// Connect WebSocket — uses server-side rendered terminal endpoint
		{
			...
			ws.onopen = () => {
				...
				// Auto-launch Claude (Phoenix) for project tabs — ALWAYS in dev
				if (projectName && projectName !== 'Shell') {
					setTimeout(async () => {
	...
	function renderChatText(text) {
		...
		html = html.replace(/\[Image:?\s*source:?\s*[^\]]*phoenix-clipboard[/\\](clipboard_\d+\.\w+)[^\]]*\]/gi,
			(_, filename) => `<img src="/clipboard/${filename}" style="${imgStyle}" onclick="window.open('/clipboard/${filename}','_blank')" />`);
		// Also match raw paths like C:/WINDOWS/TEMP/phoenix-clipboard/clipboard_123.png
		html = html.replace(/(C:[/\\](?:WINDOWS[/\\]TEMP|Users[^\s]*)[/\\]phoenix-clipboard[/\\](clipboard_\d+\.\w+))/gi,
	...
	async function respondApproval(id, response) {
		try {
			...
			console.error('[Phoenix] Approval response failed:', err);
		}
	}
			...
			if (item.type.startsWith('image/')) {
				...
				reader.onload = async () => {
					...
					try {
						...
						console.error('[Phoenix] Image paste failed:', err);
					}
				};
	...
	async function loadTestSuites() {
		try {
			...
			const serverSuites = (data.suites || []).map(s => {
				...
				return { id: s.id, name: s.name, description: s.description, tests, group: s.group || 'Phoenix Core', runInAll: s.runInAll !== false };
			});
			...
			try {
				...
				if (sessions.length > 0) {
					for (const s of sessions) {
						...
						await createTab(s.id, s.project || 'Shell', s.cwd || '%USERPROFILE%\\Desktop', pid, true);
						reconnected = true;
					}
				}
			} catch (e) {
...
<div class="toolbar">
	<select class="project-select" onchange={(e) => {
		...
		if (val === '__shell__') {
			createTab('dev-dash-shell-' + Date.now(), 'Shell', '%USERPROFILE%\\Desktop', null, false);
		} else {
...
{#snippet panelContent(currentTab)}
	...
	{:else if currentTab === 'setup'}
		<div class="setup-guide">
			<div class="setup-title">How to Use Phoenix</div>
	...
	{:else if currentTab === 'tests'}
		<div class="tests-panel" style="padding:8px">
			...
			{:else}
				<!-- App group dropdown -->
				{@const groups = [...new Set(testSuites.map(s => s.group || 'Phoenix Core'))]}
				...
				{#each testSuites.filter(s => (s.group || 'Phoenix Core') === testGroup) as suite, si}
