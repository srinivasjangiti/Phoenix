<script>
	...
	// v1: indicator + auto-open Phoenix thread. Voice loop (STT in / TTS out) wires in
	...
	// commander paused for breath before completing the thought, then Phoenix
		...
		fetch('/api/v1/chat', {
			...
			if (reply) {
				...
				chatMessages = [
					...
					{ id: routerJson?.phoenix_message_id  || ('cmsg_local_p_' + (nowMs + 1)), sender_id: chatThreadId === 'thread-phoenix-system' ? 'contact-phoenix-system' : 'phoenix', body: reply, body_type: 'text', created_at: nowMs + 1, metadata: routerJson?.debug ? { debug: routerJson.debug } : null },
				];
			}
	...
	// ─── Mail state ───
	let mailItems = $state([]);
	let mailFilter = $state('all'); // all | phoenix | email
	...
	onMount(() => {
		...
		document.title = callMode ? 'Call Phoenix' : view === 'contacts' ? 'Contacts' : view === 'mail' ? 'Mail' : 'Calendar';
		loadData();
		...
		// Auto-open the Phoenix system thread if launched from the terminal Call button.
		if (initialThread === 'thread-phoenix-system') {
			view = 'contacts';
			(async () => {
				try {
					// Make sure contacts list is populated so the sidebar highlights Phoenix.
					if (!contacts.length) {
						contacts = await api('/api/v1/chat/contacts');
					}
					const phoenixContact = contacts.find(c => c.id === 'contact-phoenix-system')
						|| { id: 'contact-phoenix-system', display_name: 'Phoenix', avatar_url: null };
					activeContact = phoenixContact;
					chatThreadId = 'thread-phoenix-system';
					...
					console.error('Auto-open Phoenix thread failed:', e);
				}
			})();
		}
	...
	async function sendChat() {
		...
		try {
			...
			if (chatThreadId === 'thread-phoenix-system') {
				try {
					const reply = await api('/api/v1/chat/phoenix-reply', {
						method: 'POST',
						headers: { 'Content-Type': 'application/json' },
					...
					if (reply?.reply) {
						chatMessages = [...chatMessages, {
							id: reply.reply.id,
							sender_id: 'contact-phoenix-system',
					...
					console.error('Phoenix reply failed:', e);
				}
			}
		} catch (e) {
	...
	// You speak, Phoenix listens (silence detection ends each turn), Phoenix replies,
	// then Phoenix listens again. Just like a phone call. No buttons.
	//
	// Per-turn pipeline:
	...
	//      Phoenix talks, so Phoenix doesn't transcribe its own voice).
	//   6. Resume VAD.
	...
	async function endCall() {
		...
		// Cancel any in-flight Phoenix reply audio
		if (currentAudio) {
	...
	function runVAD() {
		...
		const tick = () => {
			...
			// Don't listen while Phoenix is talking (avoid feedback)
	...
	async function handleTurn() {
		...
		try {
			...
			chatMessages = [
				...
				{
					id: routerJson?.phoenix_message_id || ('cmsg_local_p_' + (nowMs + 1)),
					sender_id: chatThreadId === 'thread-phoenix-system' ? 'contact-phoenix-system' : 'phoenix',
					body: reply,
					body_type: 'text',
					created_at: nowMs + 1,
			...
			// SpeechSynthesis so Phoenix can still talk back instead of silently
	...
	// Returns null when there's no debug trace — so non-Phoenix/legacy bubbles
					...
					{#if callActive}
						<div class="call-banner">
							...
							<span class="call-label">
								{#if isSpeaking}Phoenix speaking… {formatCallElapsed(callElapsed)}
					...
					<div class="chat-messages" bind:this={messagesEl}>
						{#each chatMessages as msg}
							{#if msg.body_type === 'pan_intuition_trace'}
								<!-- Collapsible intuition trace: Phoenix's reasoning shown inline,
								     same idea as Claude tool-call rendering. Click to expand. -->
								<details class="intuition-trace">
									<summary>{'\uD83E\uDDE0'} Phoenix intuition · {formatDate(msg.created_at)}</summary>
									...
									{#if dbg}
										<details class="why-trace">
											...
											<div class="why-body">
												...
												{#if dbg.mind}
													<div class="why-section">
														<div class="why-h">Phoenix's-Mind</div>
			...
			<div class="list-pane mail-list-pane">
				<div class="toolbar">
					<div class="filter-tabs">
						...
						<button class="filter-tab" class:active={(mailFilter === 'phoenix' || mailFilter === 'pan')} onclick={() => { mailFilter = 'phoenix'; loadMail(); }}>Phoenix</button>
				...
				{#each mailItems as item}
					<div class="list-row mail-row" class:unread={!item.read} class:active={selectedMail?.id === item.id} onclick={() => { selectedMail = item; }} role="button" tabindex="0">
						<span class="type-icon" class:phoenix={(item.channel === 'phoenix' || item.channel === 'pan')} class:email={item.channel === 'email'}>
							{(item.channel === 'phoenix' || item.channel === 'pan') ? '\u25C6' : '\u2709'}
						</span>
						<div class="row-info">
				...
				{#if selectedMail}
					<div class="detail-header">
						<div>
							...
							<div class="detail-sub">{formatDate(selectedMail.date)} &middot; via {selectedMail.channel === 'phoenix' || selectedMail.channel === 'pan' ? 'Phoenix' : 'Email'}</div>
						</div>
...
<style>
	...
	.type-icon.phoenix, .type-icon.pan { color: #a6e3a1; }
	...
	/* ─── 🧠 Phoenix reasoning trace under each reply (per-message debug) ─── */
	.why-trace {
