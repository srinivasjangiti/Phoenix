<script>
	...
	let unifiedFilter = $state('all');  // 'all' | 'phoenix' | 'email'
	...
	async function loadUnifiedInbox() {
		...
		try {
			...
			console.error('[Phoenix Comms] loadUnifiedInbox failed:', e);
		} finally {
	...
	async function syncEmailFromUnified() {
		...
		try {
			...
			console.error('[Phoenix Comms] syncEmail failed:', e);
		} finally {
			unifiedLoading = false;
		}
	}
	...
	async function unifiedClickMessage(msg) {
		if ((msg.type === 'phoenix' || msg.type === 'pan')) {
			// Open contact chat
	...
	async function unifiedSend() {
		...
		try {
			if (composeIsEmail) {
				...
				// Send via Phoenix — get or create thread
			...
			console.error('[Phoenix Comms] send failed:', e);
		} finally {
	...
	async function loadCommsContacts() {
		commsLoading = true;
		commsError = '';
		console.log('[Phoenix Comms] Loading contacts from', window.location.origin);
		try {
			const controller = new AbortController();
			const timeout = setTimeout(() => controller.abort(), 5000);
			const res = await fetch(`${window.location.origin}/api/v1/chat/contacts`, { signal: controller.signal });
			clearTimeout(timeout);
			console.log('[Phoenix Comms] Response status:', res.status);
			if (res.ok) {
				const data = await res.json();
				console.log('[Phoenix Comms] Got', data.length, 'contacts');
				commsContacts = data;
			} else {
			...
			console.error('[Phoenix Comms] loadCommsContacts failed:', e);
		} finally {
			commsLoading = false;
			console.log('[Phoenix Comms] Done. contacts:', commsContacts.length, 'error:', commsError);
		}
	}
	...
	async function loadWrapServices() {
		...
		} catch (e) { console.error('[Phoenix Wrap] services load failed:', e); }
	}
	...
	let brandingLogo = $state('Phoenix');
	let brandingImage = $state('');
	if (typeof window !== 'undefined') {
		const bl = localStorage.getItem('phoenix_branding_logo');
		if (bl) brandingLogo = bl;
		const bi = localStorage.getItem('phoenix_branding_image');
	...
	async function checkVersion() {
		try {
			...
			if (_knownVersion && d.version !== _knownVersion) {
				console.log('[Phoenix] Build changed, auto-reloading...');
	...
	$effect(() => {
		...
		window.addEventListener('phoenix-branding-changed', handleBrandingChange);
		return () => {
			clearInterval(iv);
			clearInterval(uiv);
			clearInterval(viv);
			window.removeEventListener('phoenix-branding-changed', handleBrandingChange);
		};
	});
</script>
...
<div class="shell">
						...
						<div class="comms-messages-view">
							...
							<div class="comms-unified-toolbar">
								<div class="comms-msg-subtabs">
									...
									<button class="comms-msg-subtab" class:active={unifiedFilter === 'phoenix'} onclick={() => { unifiedFilter = 'pan'; loadUnifiedInbox(); }}>Phoenix</button>
										...
										<button class="comms-msg-row" class:unread={!msg.read} onclick={() => unifiedClickMessage(msg)}>
											...
											<div class="comms-msg-row-body">
												<div class="comms-msg-row-top">
													...
													<span class="comms-transport-badge" class:phoenix={msg.type === 'phoenix' || (msg.type === 'phoenix' || msg.type === 'pan')} class:email={msg.type === 'email'}>{(msg.type === 'phoenix' || msg.type === 'pan') ? 'Phoenix' : 'Email'}</span>
		...
		<div class="sidebar-bottom">
			...
			{:else if !collapsed}
				<div class="server-info">
					<span class="server-label">Phoenix Server</span>
	...
	<main class="content">
		<div class="topbar">
			...
			<span class="topbar-title">{tabs.find(t => isActive(t))?.label || 'Phoenix'}</span>
...
<style>
	...
	:global(body) {
		...
		background: var(--phoenix-bg, #0a0a0f);
	...
	.sidebar {
		width: 210px;
		min-width: 210px;
		background: var(--phoenix-surface, #12121a);
		border-right: none;
		display: flex;
	...
	.sidebar-resize-handle {
		width: 4px;
		cursor: col-resize;
		background: var(--phoenix-base, #1e1e2e);
		flex-shrink: 0;
		transition: background 0.15s;
	}
	.sidebar-resize-handle:hover {
		background: var(--phoenix-accent, #89b4fa);
	}
	...
	.logo {
		...
		border-bottom: 1px solid var(--phoenix-base, #1e1e2e);
		overflow: hidden;
	}
	...
	.logo-pi {
		...
		font-size: 28px;
		font-weight: 900;
		color: var(--phoenix-logo-color, #89b4fa);
		text-shadow: var(--phoenix-logo-shadow, 0 0 18px rgba(137,180,250,0.5));
		flex-shrink: 0;
		letter-spacing: 3px;
		line-height: 1;
		user-select: none;
	...
	.logo-refresh {
		...
		padding: 2px 4px;
		font-size: 14px;
		color: var(--phoenix-faint, #6c7086);
		transition: color 0.15s, transform 0.2s;
		margin-left: auto;
	}

	.logo-refresh:hover { color: var(--phoenix-accent, #89b4fa); transform: rotate(90deg); }
	.logo-refresh:active { color: var(--phoenix-text, #cdd6f4); }
	...
	.comms-transport-badge.phoenix, .comms-transport-badge.pan {
	...
	:global([data-theme="silver-platter"] .topbar) {
		background: var(--phoenix-surface) !important;
		border-bottom: 1px solid var(--phoenix-base) !important;
	}
	:global([data-theme="silver-platter"] .topbar-title) {
		color: var(--phoenix-faint) !important;
	}

	/* Toolbar & tab bar */
	:global([data-theme="silver-platter"] .toolbar) {
		background: var(--phoenix-surface) !important; color: var(--phoenix-text) !important;
		border-bottom: 1px solid var(--phoenix-base) !important;
	}
	...
	:global([data-theme="silver-platter"] .term-tab) { color: var(--phoenix-faint) !important; }
	:global([data-theme="silver-platter"] .term-tab.active) {
		color: var(--phoenix-accent) !important; border-bottom-color: var(--phoenix-accent) !important;
	}
	:global([data-theme="silver-platter"] .add-tab) {
		background: var(--phoenix-surface2) !important; color: var(--phoenix-text) !important;
	}
	...
	:global([data-theme="silver-platter"] .right-panel) {
		background: var(--phoenix-bg) !important; border-color: var(--phoenix-base) !important;
	}
	...
	:global([data-theme="silver-platter"] .right-header) {
		background: var(--phoenix-surface) !important; color: var(--phoenix-text) !important;
		border-color: var(--phoenix-base) !important;
	}
	...
	:global([data-theme="silver-platter"] .svc-row:hover) {
		background: var(--phoenix-base) !important;
	}

	/* Center panel */
	:global([data-theme="silver-platter"] .center-tabs) {
		background: var(--phoenix-surface) !important; border-color: var(--phoenix-base) !important;
	}
	:global([data-theme="silver-platter"] .center-tab) { color: var(--phoenix-faint) !important; }
	:global([data-theme="silver-platter"] .center-tab.active) { color: var(--phoenix-accent) !important; }
	:global([data-theme="silver-platter"] .center-chat) {
		background: var(--phoenix-bg) !important; color: var(--phoenix-text) !important;
	}
	:global([data-theme="silver-platter"] .center-input-bar) {
		background: var(--phoenix-surface) !important; border-top: 1px solid var(--phoenix-base) !important;
	}
	...
	:global([data-theme="silver-platter"] .dm-bubble-phoenix),
	...
	:global([data-theme="silver-platter"] .msg-self) {
		background: var(--phoenix-overlay) !important; color: var(--phoenix-text) !important;
	}
	...
	:global([data-theme="vibe"] .tab-bar) {
		background: var(--phoenix-bg) !important; border-bottom: 1px solid var(--phoenix-base) !important;
	}
	:global([data-theme="vibe"] .term-tab) { color: var(--phoenix-faint) !important; }
	...
	:global([data-theme="vibe"] .center-tab) { color: var(--phoenix-faint) !important; }
	:global([data-theme="vibe"] .center-tab.active) { color: var(--phoenix-accent) !important; }
	...
	:global([data-theme="vibe"] .dm-bubble-phoenix),
	:global([data-theme="vibe"] .msg-bubble),
	:global([data-theme="vibe"] .cc-bubble) {
		background: rgba(255, 105, 180, 0.10) !important; color: var(--phoenix-text) !important;
	}
	...
	:global([data-theme="vibe"] .center-panel::before) {
		background: var(--phoenix-accent) !important;
		box-shadow: 0 0 12px var(--phoenix-glow) !important;
	}
